...
class extractor_flat {
 public :
  ...
  //EP Correction
  int    evt[NZ][NCENT_1p];
  double FCal_Et_m[NZ][NCENT_1p];//fcal mean et

  double Qw_m_FCal[NDET][NZ][NCENT_1p];
  double Qx_m_FCal[NDET][NZ][NCENT_1p][NHAR];
  double Qy_m_FCal[NDET][NZ][NCENT_1p][NHAR];
  /*
  float Qx_FCal[NSIDE+1][NHAR];
  float Qy_FCal[NSIDE+1][NHAR];
  float Qw_FCal[NSIDE+1];
  ...
  */
  ...
  TFile *fOut_vn,*fOut_doAna,*fOut_SP,*fOut_cum,*fOut_res,*fOut_mu;
  ...
  //Calibrations hists distributions
  TProfile *hQw_FCal[NDET][NZ],*hEt_vz[NZ];
  TProfile *hQx_FCal[NDET][NZ][NHAR];
  TProfile *hQy_FCal[NDET][NZ][NHAR];
  TProfile *hQx_Zdc[NSIDE+1][NHAR];
  TProfile *hQy_Zdc[NSIDE+1][NHAR];
  TH2F *hQ2D_Zdc[NCENT_1p][NSIDE+1][NHAR];
  TProfile *hFlatCos_Zdc[NCENT_1p][NSIDE+1][NHAR];
  TProfile *hFlatSin_Zdc[NCENT_1p][NSIDE+1][NHAR];

  TProfile *hQxS_FCal[NDET][NZ][NHAR];
  TProfile *hQyS_FCal[NDET][NZ][NHAR];
  TH1D *hqx_FCal[NDET][NZ][NHAR];
  TH1D *hqy_FCal[NDET][NZ][NHAR];
  TH1D *hqxS_FCal[NDET][NZ][NHAR];
  TH1D *hqyS_FCal[NDET][NZ][NHAR];
  TH2F* hevts;
  ...
  TProfile *hMu_Vn_FCal_pt[NCENT+1][NSIDE+1][NHAR],*hMu_Vn_FCal_cent[NPT_Mu][NSIDE+1][NHAR];
  TProfile *hMu_Vn_Zdc_eta[NCENT+1][NSIDE+1][NHAR],*hMu_Vn_Zdc_cent[NETA_Mu][NSIDE+1][NHAR];
  ...
  void Exec_mu(int entSel = 1);
  void ReadHistos_mu();
  void Init_mu();
  void Run_mu();
  void FillGlobal_mu();
  void FillTrack_mu(float Psi_FCal[3][4],float Psi_Zdc[3][4]);
  void SaveHistos_mu();
  ...
  int get_ptBin_Mu(double pt);
  int get_etaBin_Mu(double eta);
  ...
  // Declaration of leaf types for input TTree
  UInt_t          RunNumber;
  UInt_t          EventNumber;
  UInt_t          lbn;
  Int_t           vx_n;
  ...
  Int_t           ntight;
  Int_t           trk_n;
  UInt_t          trk_data[MAXTRK];   //[trk_n]
  Float_t         Qw_FCal[16];
  Float_t         Qx_FCal[16][4];
  Float_t         Qy_FCal[16][4];
  Float_t         Qw_Zdc[3];
  Float_t         Qx_Zdc[3][4];
  Float_t         Qy_Zdc[3][4];
  Int_t           Zdc_n;
  ...
  Int_t           Mu_n;
  vector<int>     *Mu_Selector;
  vector<float>   *Mu_pt;
  vector<float>   *Mu_eta;
  vector<float>   *Mu_phi;
  vector<float>   *Mu_eloss;
  vector<float>   *Mu_ms_phi;
  vector<float>   *Mu_ms_qoverp;
  vector<float>   *Mu_ms_theta;
  vector<bool>    *Mu_isMatchTrigger;
  vector<bool>    *Mu_isMatchEF_MU2;
  vector<bool>    *Mu_isMatchEF_MU4;
  vector<bool>    *Mu_isMatchEF_MU6;

  //Extra mycode
  Int_t mbtime_countA;
  Int_t mbtime_countC;
  Float_t trk_pt[MAXTRK];
  Float_t trk_eta[MAXTRK];
  Float_t trk_phi[MAXTRK];
  Int_t trk_phibin[MAXTRK];
  Int_t trk_etabin[MAXTRK];
  Int_t trk_ptbin[MAXTRK];
  Int_t trk_charge[MAXTRK];
  Int_t trk_Quality2[MAXTRK];
