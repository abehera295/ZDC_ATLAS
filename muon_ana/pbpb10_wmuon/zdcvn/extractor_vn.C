...
void extractor_flat::FillTrack_vn(float Psi_FCal[3][4],float Psi_Zdc[3][4]) {
  ...
  for(int itrk=0; itrk<trk_n; itrk++){
    ...
    */
    ...
    /*
    //For 2015 Pb+Pb
    double temp_trkEff = effTool->detTrkEff(FCal_Et,trk_pt[itrk],trk_eta[itrk]);
    double temp_trkFak = effTool->detTrkFak(FCal_Et,trk_pt[itrk],trk_eta[itrk]);
    //double temp_trkFla = effTtool->detFlat(m_zPriVtx,m_centrality,m_trkChg[iT],m_trkPt[iT],m_trkEta[iT],m_trkPhi[iT]);
    double temp_trkWei = (1.-temp_trkFak)/temp_trkEff;
    trk_eff[itrk]=1./temp_trkEff;
    */
    //double ptgev = trk_pt[itrk]/1000.0;
    double ptgev=trk_pt[itrk];
    int ptBin  = get_ptBin(trk_pt[itrk]);       if (ptBin  < 0) continue;
    ...
    //trk_wei[itrk]*=trk_eff[itrk];//For 2010 and 2011
    //trk_wei[itrk]*=temp_trkWei;//For 2015
...
void extractor_flat::Init_vn() {
  ...
  */
  ...
  for (int icent=0; icent<NCENT; icent++) {
	...
	h_Qx[icent][iside][ihar] = new TH1D(name, "", 2000, -1000, 1000);
	sprintf(name, "h_Qy_cent%d_side%d_har%d", icent, iside, ihar);
	h_Qy[icent][iside][ihar] = new TH1D(name, "", 2000, -1000, 1000);
            
	sprintf(name, "h_QxC_cent%d_side%d_har%d", icent, iside, ihar);
	h_QxC[icent][iside][ihar] = new TH1D(name, "", 2000, -1000, 1000);
	sprintf(name, "h_QyC_cent%d_side%d_har%d", icent, iside, ihar);
	h_QyC[icent][iside][ihar] = new TH1D(name, "", 2000, -1000, 1000);
      }
    }
  }
  /*
  ...
  */
  ...
  for (int icent=0; icent<NCENT; icent++) {
	...
	hVn_FCal_fg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+4,0.-0.5,NETA+4-0.5); 
	sprintf(name,"hV%d_FCal_bg_cent%d_side%d",ihar+1,icent,iside);
	hVn_FCal_bg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+4,0.-0.5,NETA+4-0.5); 
	sprintf(name,"hV%d_Zdc_fg_cent%d_side%d",ihar+1,icent,iside);
	hVn_Zdc_fg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+4,0.-0.5,NETA+4-0.5); 
	sprintf(name,"hV%d_Zdc_bg_cent%d_side%d",ihar+1,icent,iside);
	hVn_Zdc_bg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+4,0.-0.5,NETA+4-0.5); 
	/*
	//Vn Sin
  ...
  */
  //Track histograms
  hNTrk    = new TH1D("hNTrk", "", 5000, 0, 5000);
  hPt = new TH1D("hPt","",1000,0.,25.); 
  hPhi = new TH1D("hPhi","",50,0.,7); 
  hPhi_wei = new TH1D("hPhi_wei","",50,0.,7.);
  hEta = new TH1D("hEta","",100,-2.5,2.5); 
  hTrk_w = new TH1D("hTrk_w","",1000,-10.,10.);
  hTrk_wei = new TH1D("hTrk_wei","",1000,-5.,5.);
  for(int icent=0;icent<NCENT;icent++){
...
void extractor_flat::SaveHistos_vn() {
  ...
  */
  ...
  //hEtaPhi_wei->Write();
  hQw_FCal[0][0]->Write();
  hQx_FCal[0][0][0]->Write();
  hQy_FCal[0][0][0]->Write();
  hFlatCos_Zdc[0][0][0]->Write();
  hFlatSin_Zdc[0][0][0]->Write();
  fOut_vn->Close();
}
