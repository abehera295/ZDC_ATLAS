...
extractor_flat::extractor_flat(string filelist,int rno, int iev, int nevts) {
  ...
  if(RNum!=0){
    ...
    //Q-vectors
    fChain->SetBranchAddress("Qw_FCal", Qw_FCal);
    fChain->SetBranchAddress("Qx_FCal", Qx_FCal);
    fChain->SetBranchAddress("Qy_FCal", Qy_FCal);
    ...
    fChain->SetBranchAddress("Mu_Selector", &Mu_Selector);
    fChain->SetBranchAddress("Mu_pt", &Mu_pt);
    fChain->SetBranchAddress("Mu_eta", &Mu_eta);
    fChain->SetBranchAddress("Mu_phi", &Mu_phi);
    fChain->SetBranchAddress("Mu_eloss", &Mu_eloss);
    fChain->SetBranchAddress("Mu_ms_phi", &Mu_ms_phi);
    fChain->SetBranchAddress("Mu_ms_qoverp", &Mu_ms_qoverp);
    fChain->SetBranchAddress("Mu_ms_theta", &Mu_ms_theta);
    fChain->SetBranchAddress("Mu_isMatchTrigger", &Mu_isMatchTrigger);
    fChain->SetBranchAddress("Mu_isMatchEF_MU2", &Mu_isMatchEF_MU2);
    fChain->SetBranchAddress("Mu_isMatchEF_MU4", &Mu_isMatchEF_MU4);
    fChain->SetBranchAddress("Mu_isMatchEF_MU6", &Mu_isMatchEF_MU6);
  }
  set_ptBins ();
  set_etaBins();
...
void extractor_flat::Exec_mu(int entSel) {
  TStopwatch *tt=new TStopwatch();
  tt->Start();
  ...
  fOut_mu = new TFile(outname.c_str(),"recreate");
  effTool = new Tool();
  ReadHistos_mu();
  Init_mu();
  Run_mu();
...
int extractor_flat::get_ptBin_Mu(double ptVal){
  int bin=-1;
  if (ptVal < pt_mat_Mu[0]) return bin;

  for(int ipt=0;ipt<NPT_Mu;ipt++){
    if(ptVal < pt_mat_Mu[ipt+1]){
      bin = ipt;
      break;
    }
  }
...
int extractor_flat::get_etaBin_Mu(double etaVal){
  int bin=-1;
  if (etaVal < eta_mat_Mu[0]) return bin;

  for(int ieta=0;ieta<NETA_Mu;ieta++){
    if(etaVal < eta_mat_Mu[ieta+1]){
      bin = ieta;
      break;
    }
  }
