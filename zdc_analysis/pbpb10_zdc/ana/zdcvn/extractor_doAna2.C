...
void extractor_flat::ReadHistos_doAna() {
  ...
  //TFile *fIn_doAna=new TFile("./Output_vn2_old.root","read");
  TFile *fIn_doAna=new TFile("./Output_vn2.root","read");
  ...
  */
  for (int icent=0; icent<NCENT; icent++) {
    sprintf(name,"hRes_FCal_cos_fg_cent%d",icent);
    hRes_FCal_cos_fg[icent]=(TProfile*)fIn_doAna->Get(name);
    sprintf(name,"hRes_FCal_sin_fg_cent%d",icent);
    hRes_FCal_sin_fg[icent]=(TProfile*)fIn_doAna->Get(name);
    sprintf(name,"hRes_Zdc_cos_fg_cent%d",icent);
    hRes_Zdc_cos_fg[icent]=(TProfile*)fIn_doAna->Get(name);
    sprintf(name,"hRes_Zdc_sin_fg_cent%d",icent);
    hRes_Zdc_sin_fg[icent]=(TProfile*)fIn_doAna->Get(name);

    sprintf(name,"hRes_FCal_cos_bg_cent%d",icent);
    hRes_FCal_cos_bg[icent]=(TProfile*)fIn_doAna->Get(name);
    sprintf(name,"hRes_FCal_sin_bg_cent%d",icent);
    hRes_FCal_sin_bg[icent]=(TProfile*)fIn_doAna->Get(name);
    sprintf(name,"hRes_Zdc_cos_bg_cent%d",icent);
    hRes_Zdc_cos_bg[icent]=(TProfile*)fIn_doAna->Get(name);
    sprintf(name,"hRes_Zdc_sin_bg_cent%d",icent);
    hRes_Zdc_sin_bg[icent]=(TProfile*)fIn_doAna->Get(name);
  }
  //Cumulant Hists
...
void extractor_flat::Run_doAna() {
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
    for (int iside=0; iside<NSIDE+1; iside++) {
      ...
      sprintf(name,"hV%dS_Zdc_allCent_fg_side%d",ihar+1,iside);
      hVnS_Zdc_allCent_fg[iside][ihar]  = (TProfile2D*)hVnS_Zdc_fg[1][iside][ihar]->Clone(name);
      sprintf(name,"hV%dS_Zdc_allCent_bg_side%d",ihar+1,iside);
      hVnS_Zdc_allCent_bg[iside][ihar]  = (TProfile2D*)hVnS_Zdc_bg[1][iside][ihar]->Clone(name);
	...
	hVnS_Zdc_allCent_fg[iside][ihar]->Add(hVnS_Zdc_fg[icent][iside][ihar],1);
	hVnS_Zdc_allCent_bg[iside][ihar]->Add(hVnS_Zdc_bg[icent][iside][ihar],1);
      }
    }
  }
  sprintf(name, "hRes_Zdc_cos_allCent_fg");
  hRes_Zdc_cos_allCent_fg=(TProfile*)hRes_Zdc_cos_fg[1]->Clone(name);
  sprintf(name, "hRes_Zdc_sin_allCent_fg");
  hRes_Zdc_sin_allCent_fg=(TProfile*)hRes_Zdc_sin_fg[1]->Clone(name);
  sprintf(name, "hRes_Zdc_cos_allCent_bg");
  hRes_Zdc_cos_allCent_bg=(TProfile*)hRes_Zdc_cos_bg[1]->Clone(name);
  sprintf(name, "hRes_Zdc_sin_allCent_bg");
  hRes_Zdc_sin_allCent_bg=(TProfile*)hRes_Zdc_sin_bg[1]->Clone(name);
  for(int icent=2; icent<9; icent++){
    hRes_Zdc_cos_allCent_fg->Add(hRes_Zdc_cos_fg[icent],1);
    hRes_Zdc_sin_allCent_fg->Add(hRes_Zdc_sin_fg[icent],1);
    hRes_Zdc_cos_allCent_bg->Add(hRes_Zdc_cos_bg[icent],1);
    hRes_Zdc_sin_allCent_bg->Add(hRes_Zdc_sin_bg[icent],1);
  }
  ...
  std::vector<double> res_vec_FCal = discFourrier(hDphi_FCal_rat[icent][ihar],0);
  hResCent_mix_FCal[ihar]->SetBinContent(icent+1, res_vec_FCal.at(0) );
  hResCent_mix_FCal[ihar]->SetBinError  (icent+1, res_vec_FCal.at(1) );
  ...
  std::vector<double> res_vec_Zdc = discFourrier(hDphi_Zdc_rat[icent][0],ihar);
  hResCent_mix_Zdc[ihar]->SetBinContent(icent+1, res_vec_Zdc.at(0) );
  hResCent_mix_Zdc[ihar]->SetBinError  (icent+1, res_vec_Zdc.at(1) );
  }
  ...
  std::vector<double> res_vec_Zdc = discFourrier(hDphi_Zdc_allCent_rat[0],ihar);
  hRes_mix_Zdc_allCent->SetBinContent(ihar+1, res_vec_Zdc.at(0) );
  hRes_mix_Zdc_allCent->SetBinError  (ihar+1, res_vec_Zdc.at(1) );
  }
  ...
  for (int icent=0; icent<NCENT; icent++) {
	...
	for (int ieta=0; ieta<NETA+2; ieta++) {
	  for (int ipt=0; ipt<NPT; ipt++) {
	    ...
	    hVn_FCal_pt_res_fg[icent][iside][ihar][ieta]->SetBinError  (ipt+1, vnF/reso_2sub*sqrt( pow(vnF_e/vnF,2) + pow(reso_2sub_e/reso_2sub,2)));
	    ...
	    hVn_FCal_pt_res_bg[icent][iside][ihar][ieta]->SetBinError  (ipt+1, vnB/reso_2sub*sqrt( pow(vnB_e/vnB,2) + pow(reso_2sub_e/reso_2sub,2)));
	...
	for (int ipt=0; ipt<NPT+1; ipt++) {
	  for (int ieta=0; ieta<NETA; ieta++) {
	    ...
	    hVn_FCal_eta_res_fg[icent][iside][ihar][ipt]->SetBinError  (ieta+1, vnF/reso_2sub*sqrt( pow(vnF_e/vnF,2) + pow(reso_2sub_e/reso_2sub,2)));
	    ...
	    hVn_FCal_eta_res_bg[icent][iside][ihar][ipt]->SetBinContent(ieta+1, vnB/reso_2sub);
	...
	for (int ieta=0; ieta<NETA+2; ieta++) {
	  for (int ipt=0; ipt<NPT+1; ipt++) {
	    ...
	    hVn_FCal_cent_res_fg[iside][ihar][ipt][ieta]->SetBinError  (icent+1, vnF/reso_2sub*sqrt( pow(vnF_e/vnF,2) + pow(reso_2sub_e/reso_2sub,2)));
	    ...
	    hVn_FCal_cent_res_bg[iside][ihar][ipt][ieta]->SetBinError  (icent+1, vnB/reso_2sub*sqrt( pow(vnB_e/vnB,2) + pow(reso_2sub_e/reso_2sub,2)));
...
void extractor_flat::doAna_Zdc() {
  for (int icent=0; icent<NCENT; icent++) {
    for (int ihar=0; ihar<NHAR; ihar++) {
      double res_cos=hRes_Zdc_cos_fg[icent]->GetBinContent(ihar+1);
      double res_cos_e=hRes_Zdc_cos_fg[icent]->GetBinError(ihar+1);
      double res_sin=hRes_Zdc_sin_fg[icent]->GetBinContent(ihar+1);
      double res_sin_e=hRes_Zdc_sin_fg[icent]->GetBinError(ihar+1);
      res_cos_e=0.5*pow(fabs(res_cos),-0.5)*res_cos_e;
      res_cos=sqrt(fabs(res_cos));
      res_sin_e=0.5*pow(fabs(res_sin),-0.5)*res_sin_e;
      res_sin=sqrt(fabs(res_sin));

      double res_cos_ful_e=sqrt(2.)*0.5*pow(fabs(res_cos),-0.5)*res_cos_e;
      double res_cos_ful=sqrt(2.)*sqrt(fabs(res_cos));
      double res_sin_ful_e=sqrt(2.)*0.5*pow(fabs(res_sin),-0.5)*res_sin_e;
      double res_sin_ful=sqrt(2.)*sqrt(fabs(res_sin));

      double res_C[NSIDE+1],res_S[NSIDE+1];
      double res_C_e[NSIDE+1],res_S_e[NSIDE+1];
      res_C[0]=res_cos,res_S[0]=res_sin,res_C_e[0]=res_cos_e,res_S_e[0]=res_sin_e;
      res_C[1]=res_cos,res_S[1]=res_sin,res_C_e[1]=res_cos_e,res_S_e[1]=res_sin_e;
      res_C[2]=res_cos_ful,res_S[2]=res_sin_ful,res_C_e[2]=res_cos_ful_e,res_S_e[2]=res_sin_ful_e;
      double res_e,res_e_1,res_e_2;
      //1. As a function of pt
      for (int ieta=0; ieta<NETA+2; ieta++) {
	for (int ipt=0; ipt<NPT; ipt++) {
	  double vnF[NSIDE+1],vnF_e[NSIDE+1],vnSF[NSIDE+1],vnSF_e[NSIDE+1];
	  double vnB[NSIDE+1],vnB_e[NSIDE+1],vnSB[NSIDE+1],vnSB_e[NSIDE+1];
	  for (int iside=0; iside<NSIDE+1; iside++) {
	    vnF[iside]=hVn_Zdc_fg[icent][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	    vnF_e[iside]=hVn_Zdc_fg[icent][iside][ihar]->GetBinError(ipt+1,ieta+1);
	    vnB[iside]=hVn_Zdc_bg[icent][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	    vnB_e[iside]=hVn_Zdc_bg[icent][iside][ihar]->GetBinError(ipt+1,ieta+1);
	    vnSF[iside]=hVnS_Zdc_fg[icent][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	    vnSF_e[iside]=hVnS_Zdc_fg[icent][iside][ihar]->GetBinError(ipt+1,ieta+1);
	    vnSB[iside]=hVnS_Zdc_bg[icent][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	    vnSB_e[iside]=hVnS_Zdc_bg[icent][iside][ihar]->GetBinError(ipt+1,ieta+1);
	    
	    if(ihar>0){vnF[iside]=fabs(vnF[iside]);vnB[iside]=fabs(vnB[iside]);}
	    if(ihar>0){vnSF[iside]=fabs(vnSF[iside]);vnSB[iside]=fabs(vnSB[iside]);}
	  
	    //Foreground
	    hVn_Zdc_pt_raw_fg[icent][iside][ihar][ieta]->SetBinContent(ipt+1, vnF[iside]+vnSF[iside]  );
	    hVn_Zdc_pt_raw_fg[icent][iside][ihar][ieta]->SetBinError(ipt+1, sqrt(pow(vnF_e[iside],2)+pow(vnSF_e[iside],2)) );
	    hVn_Zdc_pt_res_fg[icent][iside][ihar][ieta]->SetBinContent(ipt+1, (vnF[iside]/res_C[iside] +vnSF[iside]/res_S[iside])/sqrt(2.) );
	    hVn_Zdc_pt_res_fg[icent][iside][ihar][ieta]->SetBinError(ipt+1,0.);
	    res_e_1=fabs(vnF[iside]/res_C[iside])*sqrt(pow(vnF_e[iside]/vnF[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
	    res_e_2=fabs(vnSF[iside]/res_S[iside])*sqrt(pow(vnSF_e[iside]/vnSF[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
	    res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	    hVn_Zdc_pt_res_fg[icent][iside][ihar][ieta]->SetBinError(ipt+1, res_e);
	    
	    //Background
	    hVn_Zdc_pt_raw_bg[icent][iside][ihar][ieta]->SetBinContent(ipt+1, vnB[iside]+vnSB[iside]  );
	    hVn_Zdc_pt_raw_bg[icent][iside][ihar][ieta]->SetBinError(ipt+1, sqrt(pow(vnB_e[iside],2)+pow(vnSB_e[iside],2)) );
	    hVn_Zdc_pt_res_bg[icent][iside][ihar][ieta]->SetBinContent(ipt+1, (vnB[iside]/res_C[iside] +vnSB[iside]/res_S[iside])/sqrt(2.) );
	    res_e_1=fabs(vnB[iside]/res_C[iside])*sqrt(pow(vnB_e[iside]/vnB[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
	    res_e_2=fabs(vnSB[iside]/res_S[iside])*sqrt(pow(vnSB_e[iside]/vnSB[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
	    res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	    //hVn_Zdc_pt_res_bg[icent][iside][ihar][ieta]->SetBinError(ipt+1, res_e);
	  }
	}
      }
	...
	for (int ieta=0; ieta<NETA; ieta++) {
	  ...
	  for (int iside=0; iside<NSIDE+1; iside++) {
	    ...
	    //Foreground
	    hVn_Zdc_eta_raw_fg[icent][iside][ihar][ipt]->SetBinContent(ieta+1, vnF[iside]+vnSF[iside]  );
	    hVn_Zdc_eta_raw_fg[icent][iside][ihar][ipt]->SetBinError(ieta+1, sqrt(pow(vnF_e[iside],2)+pow(vnSF_e[iside],2)) );
	    hVn_Zdc_eta_res_fg[icent][iside][ihar][ipt]->SetBinContent(ieta+1, (vnF[iside]/res_C[iside] +vnSF[iside]/res_S[iside])/sqrt(2.) );
	    hVn_Zdc_eta_res_fg[icent][iside][ihar][ipt]->SetBinError(ieta+1,0.);
	    res_e_1=fabs(vnF[iside]/res_C[iside])*sqrt(pow(vnF_e[iside]/vnF[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
	    res_e_2=fabs(vnSF[iside]/res_S[iside])*sqrt(pow(vnSF_e[iside]/vnSF[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
	    res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	    hVn_Zdc_eta_res_fg[icent][iside][ihar][ipt]->SetBinError(ieta+1, res_e);
	    
	    //Background
	    hVn_Zdc_eta_raw_bg[icent][iside][ihar][ipt]->SetBinContent(ieta+1, vnB[iside]+vnSB[iside]  );
	    hVn_Zdc_eta_raw_bg[icent][iside][ihar][ipt]->SetBinError(ieta+1, sqrt(pow(vnB_e[iside],2)+pow(vnSB_e[iside],2)) );
	    hVn_Zdc_eta_res_bg[icent][iside][ihar][ipt]->SetBinContent(ieta+1, (vnB[iside]/res_C[iside] +vnSB[iside]/res_S[iside])/sqrt(2.) );
	    res_e_1=fabs(vnB[iside]/res_C[iside])*sqrt(pow(vnB_e[iside]/vnB[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
	    res_e_2=fabs(vnSB[iside]/res_S[iside])*sqrt(pow(vnSB_e[iside]/vnSB[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
	    res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	    //hVn_Zdc_eta_res_bg[icent][iside][ihar][ipt]->SetBinError(ieta+1, res_e);
	  }
	}
      }

      //3. As a function of centrality
      for (int ieta=0; ieta<NETA+2; ieta++) {
	for (int ipt=0; ipt<NPT+1; ipt++) {
	  double vnF[NSIDE+1],vnF_e[NSIDE+1],vnSF[NSIDE+1],vnSF_e[NSIDE+1];
	  double vnB[NSIDE+1],vnB_e[NSIDE+1],vnSB[NSIDE+1],vnSB_e[NSIDE+1];
	  for (int iside=0; iside<NSIDE+1; iside++) {
	    ...
	    //Foreground
	    hVn_Zdc_cent_raw_fg[iside][ihar][ipt][ieta]->SetBinContent(icent+1, vnF[iside]+vnSF[iside]  );
	    hVn_Zdc_cent_raw_fg[iside][ihar][ipt][ieta]->SetBinError(icent+1, sqrt(pow(vnF_e[iside],2)+pow(vnSF_e[iside],2)) );
	    hVn_Zdc_cent_res_fg[iside][ihar][ipt][ieta]->SetBinContent(icent+1, (vnF[iside]/res_C[iside] +vnSF[iside]/res_S[iside])/sqrt(2.) );
	    hVn_Zdc_cent_res_fg[iside][ihar][ipt][ieta]->SetBinError(icent+1,0.);
	    res_e_1=fabs(vnF[iside]/res_C[iside])*sqrt(pow(vnF_e[iside]/vnF[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
	    res_e_2=fabs(vnSF[iside]/res_S[iside])*sqrt(pow(vnSF_e[iside]/vnSF[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
	    res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	    hVn_Zdc_cent_res_fg[iside][ihar][ipt][ieta]->SetBinError(icent+1, res_e);
	    
	    //Background
	    hVn_Zdc_cent_raw_bg[iside][ihar][ipt][ieta]->SetBinContent(icent+1, vnB[iside]+vnSB[iside]  );
	    hVn_Zdc_cent_raw_bg[iside][ihar][ipt][ieta]->SetBinError(icent+1, sqrt(pow(vnB_e[iside],2)+pow(vnSB_e[iside],2)) );
	    hVn_Zdc_cent_res_bg[iside][ihar][ipt][ieta]->SetBinContent(icent+1, (vnB[iside]/res_C[iside] +vnSB[iside]/res_S[iside])/sqrt(2.) );
	    res_e_1=fabs(vnB[iside]/res_C[iside])*sqrt(pow(vnB_e[iside]/vnB[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
	    res_e_2=fabs(vnSB[iside]/res_S[iside])*sqrt(pow(vnSB_e[iside]/vnSB[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
	    res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	    //hVn_Zdc_cent_res_bg[iside][ihar][ipt][ieta]->SetBinError(icent+1, res_e);
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
    double res_cos=hRes_Zdc_cos_allCent_fg->GetBinContent(ihar+1);
    double res_cos_e=hRes_Zdc_cos_allCent_fg->GetBinError(ihar+1);
    double res_sin=hRes_Zdc_sin_allCent_fg->GetBinContent(ihar+1);
    double res_sin_e=hRes_Zdc_sin_allCent_fg->GetBinError(ihar+1);
    res_cos_e=0.5*pow(fabs(res_cos),-0.5)*res_cos_e;
    res_cos=sqrt(fabs(res_cos));
    res_sin_e=0.5*pow(fabs(res_sin),-0.5)*res_sin_e;
    res_sin=sqrt(fabs(res_sin));
	...
	double vnB[NSIDE+1],vnB_e[NSIDE+1],vnSB[NSIDE+1],vnSB_e[NSIDE+1];
	for (int iside=0; iside<NSIDE+1; iside++) {
	  vnF[iside]=hVn_Zdc_allCent_fg[iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  vnF_e[iside]=hVn_Zdc_allCent_fg[iside][ihar]->GetBinError(ipt+1,ieta+1);
	  vnB[iside]=hVn_Zdc_allCent_bg[iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  vnB_e[iside]=hVn_Zdc_allCent_bg[iside][ihar]->GetBinError(ipt+1,ieta+1);
	  vnSF[iside]=hVnS_Zdc_allCent_fg[iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  vnSF_e[iside]=hVnS_Zdc_allCent_fg[iside][ihar]->GetBinError(ipt+1,ieta+1);
	  vnSB[iside]=hVnS_Zdc_allCent_bg[iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  vnSB_e[iside]=hVnS_Zdc_allCent_bg[iside][ihar]->GetBinError(ipt+1,ieta+1);
	  
	  if(ihar>0){vnF[iside]=fabs(vnF[iside]);vnB[iside]=fabs(vnB[iside]);}
	  if(ihar>0){vnSF[iside]=fabs(vnSF[iside]);vnSB[iside]=fabs(vnSB[iside]);}

	  //Foreground
	  hVn_Zdc_pt_raw_allCent_fg[iside][ihar][ieta]->SetBinContent(ipt+1, vnF[iside]+vnSF[iside]  );
	  hVn_Zdc_pt_raw_allCent_fg[iside][ihar][ieta]->SetBinError(ipt+1, sqrt(pow(vnF_e[iside],2)+pow(vnSF_e[iside],2)) );
	  hVn_Zdc_pt_res_allCent_fg[iside][ihar][ieta]->SetBinContent(ipt+1, (vnF[iside]/res_C[iside] +vnSF[iside]/res_S[iside])/sqrt(2.) );
	  hVn_Zdc_pt_res_allCent_fg[iside][ihar][ieta]->SetBinError(ipt+1,0.);
	  res_e_1=fabs(vnF[iside]/res_C[iside])*sqrt(pow(vnF_e[iside]/vnF[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
	  res_e_2=fabs(vnSF[iside]/res_S[iside])*sqrt(pow(vnSF_e[iside]/vnSF[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
	  res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	  hVn_Zdc_pt_res_allCent_fg[iside][ihar][ieta]->SetBinError(ipt+1, res_e);

	  //Background
	  hVn_Zdc_pt_raw_allCent_bg[iside][ihar][ieta]->SetBinContent(ipt+1, vnB[iside]+vnSB[iside]  );
	  hVn_Zdc_pt_raw_allCent_bg[iside][ihar][ieta]->SetBinError(ipt+1, sqrt(pow(vnB_e[iside],2)+pow(vnSB_e[iside],2)) );
	  hVn_Zdc_pt_res_allCent_bg[iside][ihar][ieta]->SetBinContent(ipt+1, (vnB[iside]/res_C[iside] +vnSB[iside]/res_S[iside])/sqrt(2.) );
	  res_e_1=fabs(vnB[iside]/res_C[iside])*sqrt(pow(vnB_e[iside]/vnB[iside],2)+pow(res_C_e[iside]/res_C[iside],2));
          res_e_2=fabs(vnSB[iside]/res_S[iside])*sqrt(pow(vnSB_e[iside]/vnSB[iside],2)+pow(res_S_e[iside]/res_S[iside],2));
          res_e=(1./sqrt(2.)) * sqrt(pow(res_e_1,2)+pow(res_e_2,2));
	  //hVn_Zdc_pt_res_allCent_bg[iside][ihar][ieta]->SetBinError(ipt+1, res_e);
...
void extractor_flat::SaveHistos_doAna() {
  fOut_doAna = new TFile("Output_doAna_vn2.root", "recreate");
  fOut_doAna->cd();
  /*
    for (int icent=0; icent<NCENT; icent++) {
    for (int iside=0; iside<NSIDE+1; iside++) {
  ...
  */
  for (int icent=0; icent<NCENT; icent++) {
    hRes_FCal_cos_fg[icent]->Write();
    hRes_FCal_sin_fg[icent]->Write();
    hRes_Zdc_cos_fg[icent]->Write();
    hRes_Zdc_sin_fg[icent]->Write();

    hRes_FCal_cos_bg[icent]->Write();
    hRes_FCal_sin_bg[icent]->Write();
    hRes_Zdc_cos_bg[icent]->Write();
    hRes_Zdc_sin_bg[icent]->Write();
  }
  hRes_Zdc_cos_allCent_fg->Write();
  hRes_Zdc_sin_allCent_fg->Write();
  //Cumulant hists
  for(int ihar=0;ihar<NHAR2;ihar++){
