...
void extractor_flat::ReadHistos_3pv() {
  /*
...
void extractor_flat::GetTrackQn_3pv(){
  ntrkQ = 0;
  ...
  for(int itrk=0; itrk<trk_n; itrk++){
    ...
    hMean_pt[centb][ich][iside]->Fill(ipt,trk_pt[itrk]);
    hMean_pt[centb][NCH][iside]->Fill(ipt,trk_pt[itrk]);
    hMean_pt[centb][ich][NSIDE]->Fill(ipt,trk_pt[itrk]);
    hMean_pt[centb][NCH][NSIDE]->Fill(ipt,trk_pt[itrk]);
    
    hMean_eta[centb][ich][iside]->Fill(ieta,trk_eta[itrk]);
    hMean_eta[centb][NCH][iside]->Fill(ieta,trk_eta[itrk]);
...
void extractor_flat::FillHisto_3pv(int id0){
  ...
  */
  Fill_2PCfg_3pv(ev,ev);
  //Fill_3PCfg_3pv(ev,ev,ev);
  delete ev;
}

void extractor_flat::Fill_2PCfg_3pv(Event* ev1, Event *ev2){
  //My code
...
void extractor_flat::Fill_3PCfg_3pv(Event* ev1, Event *ev2, Event *ev3){
  //My code
  TComplex q1_ID[NCH+1][NPT+1][NSIDE+1][NETA2+1][NHAR],q1c_ID[NCH+1][NPT+1][NSIDE+1][NETA2+1][NHAR];
...
void extractor_flat::SaveHistos_3pv() {
  fOut_3pv->cd();
  for(int icent=0;icent<NCENT;icent++){
    for(int ich=0;ich<NCH+1;ich++){
      for(int iside=0;iside<NSIDE+1;iside++){
