...
void plot_atlas_v2_pt(){
  ...
  for(int ihar=0; ihar<NHAR; ihar++) {
    ...
    sprintf(name,"hV%d_Zdc_cent_res_side%d_pt%d_eta%d",ihar+1,NSIDE,NPT,NETA);
    hVn_Zdc_cent_res[ihar]=(TH1D*)fin->Get(name);
    for(int icent=0;icent<NCENT;icent++){
      sprintf(name,"hV%d_FCal_pt_res_cent%d_side%d_eta%d",ihar+1,icent,NSIDE,NETA);
      hVn_FCal_pt_res[ihar][icent]=(TH1D*)fin->Get(name);
      sprintf(name,"hV%d_Zdc_pt_res_cent%d_side%d_eta%d",ihar+2,icent,NSIDE,NETA);
