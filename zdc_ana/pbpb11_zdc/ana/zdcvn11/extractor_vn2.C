void extractor_flat::ReadHistos_vn2() {
  /*
...
void extractor_flat::Run_vn2() {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for (int ihar=0; ihar<NHAR; ihar++) {
      ...
      double dPsi_FCal = (ihar+2)*(Psi_FCal[1][ihar]-Psi_FCal[0][ihar] );
      if (dPsi_FCal < -PI) dPsi_FCal += 2*PI;
      if (dPsi_FCal >  PI) dPsi_FCal -= 2*PI;
      double dPsi_Zdc = (ihar+1)*(Psi_Zdc[1][ihar]-Psi_Zdc[0][ihar] );
      if (dPsi_Zdc < -PI) dPsi_Zdc += 2*PI;
      if (dPsi_Zdc >  PI) dPsi_Zdc -= 2*PI;
      hDphi_FCal_fg[centb][ihar]->Fill(dPsi_FCal);
      hDphi_Zdc_fg[centb][ihar]->Fill(dPsi_Zdc);
      //new
      hRes_FCal_cos_fg[centb]->Fill( ihar,cos((ihar+2)*Psi_FCal[0][ihar])*cos((ihar+2)*Psi_FCal[1][ihar]) );
      hRes_FCal_sin_fg[centb]->Fill( ihar,sin((ihar+2)*Psi_FCal[0][ihar])*sin((ihar+2)*Psi_FCal[1][ihar]) );
      hRes_Zdc_cos_fg[centb]->Fill( ihar,cos((ihar+1)*Psi_Zdc[0][0])*cos((ihar+1)*Psi_Zdc[1][0]) );
      hRes_Zdc_sin_fg[centb]->Fill( ihar,sin((ihar+1)*Psi_Zdc[0][0])*sin((ihar+1)*Psi_Zdc[1][0]) );
      /*
      //For Background 
      Pool_size =Pool_EP[centb].size();
      if(Pool_size>0){
	for (int isize=0; isize<Pool_size; isize++) {
	  
	  dPsi_FCal = (ihar+2)*( Psi_FCal[1][ihar] - (Pool_EP[centb].at(isize)->Psi_FCal[0][ihar]) );
	  if (dPsi_FCal < -PI) dPsi_FCal += 2*PI;
	  if (dPsi_FCal >  PI) dPsi_FCal -= 2*PI;
	  hDphi_FCal_bg[centb][ihar]->Fill(dPsi_FCal);

	  dPsi_FCal = (ihar+2)*( Psi_FCal[0][ihar] - (Pool_EP[centb].at(isize)->Psi_FCal[1][ihar]) );
	  if (dPsi_FCal < -PI) dPsi_FCal += 2*PI;
	  if (dPsi_FCal >  PI) dPsi_FCal -= 2*PI;
	  hDphi_FCal_bg[centb][ihar]->Fill(-dPsi_FCal);

	  dPsi_Zdc = (ihar+1)*( Psi_Zdc[1][ihar] - (Pool_EP[centb].at(isize)->Psi_Zdc[0][ihar]) );
	  if (dPsi_Zdc < -PI) dPsi_Zdc += 2*PI;
	  if (dPsi_Zdc >  PI) dPsi_Zdc -= 2*PI;
	  hDphi_Zdc_bg[centb][ihar]->Fill(dPsi_Zdc);

	  dPsi_Zdc = (ihar+1)*( Psi_Zdc[0][ihar] - (Pool_EP[centb].at(isize)->Psi_Zdc[1][ihar]) );
	  if (dPsi_Zdc < -PI) dPsi_Zdc += 2*PI;
	  if (dPsi_Zdc >  PI) dPsi_Zdc -= 2*PI;
	  hDphi_Zdc_bg[centb][ihar]->Fill(-dPsi_Zdc);
	
	  hRes_FCal_cos_bg[centb]->Fill( ihar,cos((ihar+2)*Psi_FCal[0][ihar])*cos((ihar+2)*Pool_EP[centb].at(isize)->Psi_FCal[1][ihar]) );
	  hRes_FCal_sin_bg[centb]->Fill( ihar,sin((ihar+2)*Psi_FCal[0][ihar])*sin((ihar+2)*Pool_EP[centb].at(isize)->Psi_FCal[1][ihar]) );
	  hRes_Zdc_cos_bg[centb]->Fill( ihar,cos((ihar+1)*Psi_Zdc[0][0])*cos((ihar+1)*Pool_EP[centb].at(isize)->Psi_Zdc[1][0]) );
	  hRes_Zdc_sin_bg[centb]->Fill( ihar,sin((ihar+1)*Psi_Zdc[0][0])*sin((ihar+1)*Pool_EP[centb].at(isize)->Psi_Zdc[1][0]) );

	  hRes_FCal_cos_bg[centb]->Fill( ihar,cos((ihar+2)*Pool_EP[centb].at(isize)->Psi_FCal[0][ihar])*cos((ihar+2)*Psi_FCal[1][ihar]) );
	  hRes_FCal_sin_bg[centb]->Fill( ihar,sin((ihar+2)*Pool_EP[centb].at(isize)->Psi_FCal[0][ihar])*sin((ihar+2)*Psi_FCal[1][ihar]) );
	  hRes_Zdc_cos_bg[centb]->Fill( ihar,cos((ihar+1)*Pool_EP[centb].at(isize)->Psi_Zdc[0][0])*cos((ihar+1)*Psi_Zdc[1][0]) );
	  hRes_Zdc_sin_bg[centb]->Fill( ihar,sin((ihar+1)*Pool_EP[centb].at(isize)->Psi_Zdc[0][0])*sin((ihar+1)*Psi_Zdc[1][0]) );
	}
      }
      */
    }
    FillTrack_vn2(Psi_FCal,Psi_Zdc);
    /*
    Pool_size=Pool_EP[centb].size();
    if(Pool_size>=STORE_DEP){
      delete Pool_EP[centb].at(0);
      Pool_EP[centb].erase( (Pool_EP[centb]).begin() );
    }
    Pool_EP[centb].push_back( ev );
    //delete ev;
    */
  }//Event Loop ends
...
void extractor_flat::FillGlobal_vn2() {
...
void extractor_flat::FillTrack_vn2(float Psi_FCal[3][4],float Psi_Zdc[3][4]) {
  ntrkQ = 0;
  for (int icent=0; icent<NCENT; icent++) {
    for (int iside=0; iside<NSIDE+1; iside++) {
      for (int ihar=0; ihar<NHAR; ihar++) {
  ...
  for(int itrk=0; itrk<trk_n; itrk++){
    ...
    for (int iside=0; iside<NSIDE+1; iside++) {
      for (int ihar=0; ihar<NHAR; ihar++) {
	//Fill foreground
	/*double corr1_FCal_fg = cos( (ihar+2)*(trk_phi[itrk]-Psi_FCal[iside][ihar]) )*trk_wei[itrk];
	double corr2_FCal_fg = sin( (ihar+2)*(trk_phi[itrk]-Psi_FCal[iside][ihar]) )*trk_wei[itrk];
	double corr1_Zdc_fg = cos( (ihar+1)*(trk_phi[itrk]-Psi_Zdc[iside][0]) )*trk_wei[itrk];
        double corr2_Zdc_fg = sin( (ihar+1)*(trk_phi[itrk]-Psi_Zdc[iside][0]) )*trk_wei[itrk];
	*/
	double corr1_FCal_fg = cos((ihar+2)*trk_phi[itrk]) * cos((ihar+2)*Psi_FCal[iside][ihar]) * trk_wei[itrk];
        double corr2_FCal_fg = sin((ihar+2)*trk_phi[itrk]) * sin((ihar+2)*Psi_FCal[iside][ihar]) * trk_wei[itrk];
        double corr1_Zdc_fg = cos((ihar+1)*trk_phi[itrk]) * cos((ihar+1)*Psi_Zdc[iside][0]) * trk_wei[itrk];
        double corr2_Zdc_fg = sin((ihar+1)*trk_phi[itrk]) * sin((ihar+1)*Psi_Zdc[iside][0]) * trk_wei[itrk];
	...
	if(fabs(trk_eta[itrk])<0.8) hVn_FCal_tmp_fg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr1_FCal_fg);
	...
	if(fabs(trk_eta[itrk])<0.8) hVn_FCal_tmp_fg[centb][iside][ihar]->Fill(NPT,NETA+1,corr1_FCal_fg);
	...
	if(fabs(trk_eta[itrk])<0.8)  hVnS_FCal_tmp_fg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr2_FCal_fg);
	...
	if(fabs(trk_eta[itrk])<0.8) hVnS_FCal_tmp_fg[centb][iside][ihar]->Fill(NPT,NETA+1,corr2_FCal_fg);
	...
	if(fabs(trk_eta[itrk])<0.8) hVn_Zdc_tmp_fg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr1_Zdc_fg);
	...
	if(fabs(trk_eta[itrk])<0.8) hVn_Zdc_tmp_fg[centb][iside][ihar]->Fill(NPT,NETA+1,corr1_Zdc_fg);
	...
	if(fabs(trk_eta[itrk])<0.8) hVnS_Zdc_tmp_fg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr2_Zdc_fg);
	...
	if(fabs(trk_eta[itrk])<0.8) hVnS_Zdc_tmp_fg[centb][iside][ihar]->Fill(NPT,NETA+1,corr2_Zdc_fg);
	...
	hVnW_tmp_fg[centb][iside][ihar]->Fill(ptBin,etaBin,trk_wei[itrk]);
        ...
        if(fabs(trk_eta[itrk])<0.8) hVnW_tmp_fg[centb][iside][ihar]->Fill(ptBin,NETA+1,trk_wei[itrk]);
        hVnW_tmp_fg[centb][iside][ihar]->Fill(NPT,NETA,trk_wei[itrk]);
        if(fabs(trk_eta[itrk])<0.8) hVnW_tmp_fg[centb][iside][ihar]->Fill(NPT,NETA+1,trk_wei[itrk]);
	...
	if(Pool_size>0){
	  for (int isize=0; isize<Pool_size; isize++) {
	    float Psi_FCal_temp=Pool_EP[centb].at(isize)->Psi_FCal[iside][ihar];
	    //double corr1_FCal_bg = cos( (ihar+2)*(trk_phi[itrk]-Psi_FCal_temp) )*trk_wei[itrk];
	    //double corr2_FCal_bg = sin( (ihar+2)*(trk_phi[itrk]-Psi_FCal_temp) )*trk_wei[itrk];
	    double corr1_FCal_bg = cos((ihar+2)*trk_phi[itrk]) * cos((ihar+2)*Psi_FCal_temp) * trk_wei[itrk];
	    double corr2_FCal_bg = sin((ihar+2)*trk_phi[itrk]) * sin((ihar+2)*Psi_FCal_temp) * trk_wei[itrk];
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVn_FCal_tmp_bg[centb][iside][ihar][isize]->Fill(ptBin,NETA+1,corr1_FCal_bg);
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVn_FCal_tmp_bg[centb][iside][ihar][isize]->Fill(NPT,NETA+1,corr1_FCal_bg);
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVnS_FCal_tmp_bg[centb][iside][ihar][isize]->Fill(ptBin,NETA+1,corr2_FCal_bg);
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVnS_FCal_tmp_bg[centb][iside][ihar][isize]->Fill(NPT,NETA+1,corr2_FCal_bg);
	    //hVnS_FCal_tmp_bg[centb][iside][ihar][isize]->Fill(NPT,NETA+1,corr2_FCal_bg*sign_eta);

	    float Psi_Zdc_temp=Pool_EP[centb].at(isize)->Psi_Zdc[iside][0];
	    //double corr1_Zdc_bg = cos( (ihar+1)*(trk_phi[itrk]-Psi_Zdc_temp) )*trk_wei[itrk];
	    //double corr2_Zdc_bg = sin( (ihar+1)*(trk_phi[itrk]-Psi_Zdc_temp) )*trk_wei[itrk];
	    double corr1_Zdc_bg = cos((ihar+1)*trk_phi[itrk]) * cos((ihar+1)*Psi_Zdc_temp) * trk_wei[itrk];
	    double corr2_Zdc_bg = sin((ihar+1)*trk_phi[itrk]) * sin((ihar+1)*Psi_Zdc_temp) * trk_wei[itrk];
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVn_Zdc_tmp_bg[centb][iside][ihar][isize]->Fill(ptBin,NETA+1,corr1_Zdc_bg);
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVn_Zdc_tmp_bg[centb][iside][ihar][isize]->Fill(NPT,NETA+1,corr1_Zdc_bg);
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVnS_Zdc_tmp_bg[centb][iside][ihar][isize]->Fill(ptBin,NETA+1,corr2_Zdc_bg);
	    ...
	    if(fabs(trk_eta[itrk])<0.8) hVnS_Zdc_tmp_bg[centb][iside][ihar][isize]->Fill(NPT,NETA+1,corr2_Zdc_bg);
	    ...
	    hVnW_tmp_bg[centb][iside][ihar][isize]->Fill(ptBin,etaBin,trk_wei[itrk]);
            ...
            if(fabs(trk_eta[itrk])<0.8) hVnW_tmp_bg[centb][iside][ihar][isize]->Fill(ptBin,NETA+1,trk_wei[itrk]);
            hVnW_tmp_bg[centb][iside][ihar][isize]->Fill(NPT,NETA,trk_wei[itrk]);
            if(fabs(trk_eta[itrk])<0.8) hVnW_tmp_bg[centb][iside][ihar][isize]->Fill(NPT,NETA+1,trk_wei[itrk]);
	  }
	}
	*/
      }
  ...
  for (int iside=0; iside<NSIDE+1; iside++) {
	...
	for (int ieta=0; ieta<NETA+2;ieta++){
	  double Vn_FCal=hVn_FCal_tmp_fg[centb][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  double VnS_FCal=hVnS_FCal_tmp_fg[centb][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  double Vn_Zdc=hVn_Zdc_tmp_fg[centb][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  double VnS_Zdc=hVnS_Zdc_tmp_fg[centb][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  double Vnw=hVnW_tmp_fg[centb][iside][ihar]->GetBinContent(ipt+1,ieta+1);
	  if(Vnw!=0.){
	    Vn_FCal/=Vnw;
	    VnS_FCal/=Vnw;
	    Vn_Zdc/=Vnw;
	...
	for (int ipt=0; ipt<NPT+1;ipt++){
	  for (int ieta=0; ieta<NETA+2;ieta++){
	    double Vn_FCal=hVn_FCal_tmp_bg[centb][iside][ihar][isize]->GetBinContent(ipt+1,ieta+1);
	    double VnS_FCal=hVnS_FCal_tmp_bg[centb][iside][ihar][isize]->GetBinContent(ipt+1,ieta+1);
	    double Vn_Zdc=hVn_Zdc_tmp_bg[centb][iside][ihar][isize]->GetBinContent(ipt+1,ieta+1);
	    double VnS_Zdc=hVnS_Zdc_tmp_bg[centb][iside][ihar][isize]->GetBinContent(ipt+1,ieta+1);
	    double Vnw=hVnW_tmp_bg[centb][iside][ihar][isize]->GetBinContent(ipt+1,ieta+1);
	    if(Vnw!=0.){
	      Vn_FCal/=Vnw;
	      VnS_FCal/=Vnw;
	      Vn_Zdc/=Vnw;
...
void extractor_flat::Init_vn2() {
  ...
  for (int icent=0; icent<NCENT; icent++) {
	...
	hPsi_FCal[icent][iside][ihar] = new TH1D(name, "", 100, -PI, PI);
	hPsi_FCal[icent][iside][ihar]->Sumw2();
	sprintf(name, "hPsi_Zdc_cent%d_side%d_har%d", icent, iside, ihar);
	hPsi_Zdc[icent][iside][ihar] = new TH1D(name, "", 100, -PI, PI);
  ...
  for (int icent=0; icent<NCENT; icent++) {
    ...
    hRes_FCal_cos_fg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5);
    sprintf(name,"hRes_FCal_sin_fg_cent%d",icent);
    hRes_FCal_sin_fg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5);
    sprintf(name,"hRes_Zdc_cos_fg_cent%d",icent);
    hRes_Zdc_cos_fg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5);
    sprintf(name,"hRes_Zdc_sin_fg_cent%d",icent);
    hRes_Zdc_sin_fg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5); 

    sprintf(name,"hRes_FCal_cos_bg_cent%d",icent);
    hRes_FCal_cos_bg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5);
    sprintf(name,"hRes_FCal_sin_bg_cent%d",icent);
    hRes_FCal_sin_bg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5);
    sprintf(name,"hRes_Zdc_cos_bg_cent%d",icent);
    hRes_Zdc_cos_bg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5);
    sprintf(name,"hRes_Zdc_sin_bg_cent%d",icent);
    hRes_Zdc_sin_bg[icent]=new TProfile(name,"",NHAR,0.-0.5,NHAR-0.5);
  }
  ...
  for (int icent=0; icent<NCENT; icent++) {
	...
	hVn_FCal_fg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVn_FCal_fg[icent][iside][ihar]->Sumw2();
	sprintf(name,"hV%dS_FCal_fg_cent%d_side%d",ihar+1,icent,iside);
	hVnS_FCal_fg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVnS_FCal_fg[icent][iside][ihar]->Sumw2();

	sprintf(name,"hV%d_FCal_bg_cent%d_side%d",ihar+1,icent,iside);
	hVn_FCal_bg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVn_FCal_bg[icent][iside][ihar]->Sumw2();
	sprintf(name,"hV%dS_FCal_bg_cent%d_side%d",ihar+1,icent,iside);
	hVnS_FCal_bg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVnS_FCal_bg[icent][iside][ihar]->Sumw2();

	sprintf(name,"hV%d_Zdc_fg_cent%d_side%d",ihar+1,icent,iside);
	hVn_Zdc_fg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVn_Zdc_fg[icent][iside][ihar]->Sumw2();
	sprintf(name,"hV%dS_Zdc_fg_cent%d_side%d",ihar+1,icent,iside);
	hVnS_Zdc_fg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVnS_Zdc_fg[icent][iside][ihar]->Sumw2();

	sprintf(name,"hV%d_Zdc_bg_cent%d_side%d",ihar+1,icent,iside);
	hVn_Zdc_bg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVn_Zdc_bg[icent][iside][ihar]->Sumw2();
	sprintf(name,"hV%dS_Zdc_bg_cent%d_side%d",ihar+1,icent,iside);
	hVnS_Zdc_bg[icent][iside][ihar]  = new TProfile2D(name, "",NPT+1,0.-0.5,NPT+1-0.5,NETA+2,0.-0.5,NETA+2-0.5); hVnS_Zdc_bg[icent][iside][ihar]->Sumw2();
      }
    }
  }
  ...
  /*
    ...
    for (int ihar=0; ihar<NHAR; ihar++) {
    for (int ipt=0; ipt<NPT+1; ipt++) {
    std::vector<TH1D*> hTemp_vec1_fg(NETA+2, (TH1D*)0);
    std::vector<TH1D*> hTemp_vec2_fg(NETA+2, (TH1D*)0);
    std::vector<TH1D*> hTemp_vec1_bg(NETA+2, (TH1D*)0);
    std::vector<TH1D*> hTemp_vec2_bg(NETA+2, (TH1D*)0);
    for (int ieta=0; ieta<NETA+2; ieta++) {
...
void extractor_flat::SaveHistos_vn2() {
  fOut_vn2->cd();
