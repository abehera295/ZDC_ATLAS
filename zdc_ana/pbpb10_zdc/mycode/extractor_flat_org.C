...
void extractor_flat::exec(int entSel) {
   ...
   fOut = new TFile("Output_zdcPix_flat_wtrk_local.root", "recreate");
...
void extractor_flat::FillTrackVn(std::vector<std::vector<double> > PsiF) {
   ...
   for(int j=0; j<trk_n; j++){
      ...
      if (ptBin>4) std::cout << "trk = " << j << ": pt = " << ptgev << ": ptBin = " << ptBin << ": eta = " << trk_eta[j] << ": etaBin = " << etaBin << ": hVn = " << (hVn[centb][0][0]).at(ptBin).at(etaBin)->GetName() << std::endl;

      for (int iside=0; iside<NSIDE+1; iside++) {
         for (int ihar=0; ihar<NHAR; ihar++) {
            double corr1 = cos( (ihar+1)*(trk_phi0_wrt_PV[j]-PsiF.at(iside).at(ihar)) );
...
void extractor_flat::FillFlattening(){
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (nb <=0)        {cout<<"Nbytes became <=0"<<endl;break;}
      if(jentry%10000==0) {cout<<"In FillFlattening:: event "<<jentry<<endl ;}
...
void extractor_flat::FillCalib() {
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (nb <=0)        {cout<<"Nbytes became <=0"<<endl;break;}
      if(jentry%10000==0) {cout<<"In FillCalib::  event "<<jentry<<endl ;}
