...
void extractor_flat::SetUpAll() {
         ...
         for (int ihar=0; ihar<NHAR; ihar++) {
            for (int ipt=0; ipt<NPT+1; ipt++) {
               ...
               for (int ieta=0; ieta<NETA+2; ieta++) {
                  ...
                  hTemp_vec1.at(ieta) = new TH1D(name, "", 220, -1.1, 1.1); hTemp_vec1.at(ieta)->Sumw2();
                  sprintf(name, "hV%dS_cent%d_side%d_pt%d_eta%d", ihar+1, icent, iside, ipt, ieta); 
                  hTemp_vec2.at(ieta) = new TH1D(name, "", 220, -1.1, 1.1); hTemp_vec2.at(ieta)->Sumw2();
               }
   ...
   hPhi = new TH1D("hPhi","",100,0.,7); hPhi->Sumw2();
