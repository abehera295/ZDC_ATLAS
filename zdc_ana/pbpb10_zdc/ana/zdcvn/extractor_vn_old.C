  ...
  */
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for (int ihar=0; ihar<NHAR; ihar++) {
      ...
      */
      //Psi Store creation 
      int Pool_size =Pool_EP[centb].size();
      if(Pool_size>0){
  ...
  cout<<"Finished running all jobs."<<endl;
}
...
void extractor_flat::FillTrackVn(float Psi_FCal[3][4],float Psi_Zdc[3][4]) {
  ntrkQ = 0;
  for (int icent=0; icent<NCENT; icent++) {
    for (int iside=0; iside<NSIDE+1; iside++) {
      for (int ihar=0; ihar<NHAR; ihar++) {
  ...
  for(int itrk=0; itrk<trk_n; itrk++){
    ...
    for (int iside=0; iside<NSIDE+1; iside++) {
      for (int ihar=0; ihar<NHAR; ihar++) {
	//Fill foreground
	//double corr1 = cos( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc.at(iside).at(0)) )*trk_wei[itrk];
	//double corr2 = sin( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc.at(iside).at(0)) )*trk_wei[itrk];
	//Fill foreground
	double corr1_FCal_fg = cos( (ihar+2)*(trk_phi0_wrt_PV[itrk]-Psi_FCal[iside][ihar]) )*trk_wei[itrk];
	double corr2_FCal_fg = sin( (ihar+2)*(trk_phi0_wrt_PV[itrk]-Psi_FCal[iside][ihar]) )*trk_wei[itrk];
	double corr1_Zdc_fg = cos( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc[iside][0]) )*trk_wei[itrk];
        double corr2_Zdc_fg = sin( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc[iside][0]) )*trk_wei[itrk];
	...
	if(Pool_size>0){
	  for (int isize=0; isize<Pool_size; isize++) {
	    ...
	    double corr1_FCal_bg = cos( (ihar+2)*(trk_phi0_wrt_PV[itrk]-Psi_FCal_temp) )*trk_wei[itrk];
	    double corr2_FCal_bg = sin( (ihar+2)*(trk_phi0_wrt_PV[itrk]-Psi_FCal_temp) )*trk_wei[itrk];
	    ...
	    float Psi_Zdc_temp=Pool_EP[centb].at(isize)->Psi_Zdc[iside][ihar];
	    double corr1_Zdc_bg = cos( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc_temp) )*trk_wei[itrk];
	    double corr2_Zdc_bg = sin( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc_temp) )*trk_wei[itrk];
	...
	/*
	  int str_size_FCal = (Psi_FCal_store[centb_1p][iside][ihar]).size();
	  if(str_size_FCal>0){
	  for (int isiz=0; isiz<str_size_FCal; isiz++) {
	  double Psi_FCal_temp=(Psi_FCal_store[centb_1p][iside][ihar]).at(isiz);
	  double corr1_FCal_bg = cos( (ihar+2)*(trk_phi0_wrt_PV[itrk]-Psi_FCal_temp) )*trk_wei[itrk];
	  double corr2_FCal_bg = sin( (ihar+2)*(trk_phi0_wrt_PV[itrk]-Psi_FCal_temp) )*trk_wei[itrk];
	  hVn_FCal_bg[centb][iside][ihar]->Fill(ptBin,etaBin,corr1_FCal_bg,trk_wei[itrk]);
	  hVn_FCal_bg[centb][iside][ihar]->Fill(NPT,etaBin,corr1_FCal_bg,trk_wei[itrk]);
	  hVn_FCal_bg[centb][iside][ihar]->Fill(ptBin,NETA,corr1_FCal_bg,trk_wei[itrk]);
	  hVn_FCal_bg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr1_FCal_bg*sign_eta,trk_wei[itrk]);
	  hVn_FCal_bg[centb][iside][ihar]->Fill(NPT,NETA,corr1_FCal_bg,trk_wei[itrk]);
	  hVn_FCal_bg[centb][iside][ihar]->Fill(NPT,NETA+1,corr1_FCal_bg*sign_eta,trk_wei[itrk]);

	  hVnS_FCal_bg[centb][iside][ihar]->Fill(ptBin,etaBin,corr2_FCal_bg,trk_wei[itrk]);
	  hVnS_FCal_bg[centb][iside][ihar]->Fill(NPT,etaBin,corr2_FCal_bg,trk_wei[itrk]);
	  hVnS_FCal_bg[centb][iside][ihar]->Fill(ptBin,NETA,corr2_FCal_bg,trk_wei[itrk]);
	  hVnS_FCal_bg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr2_FCal_bg*sign_eta,trk_wei[itrk]);
	  hVnS_FCal_bg[centb][iside][ihar]->Fill(NPT,NETA,corr2_FCal_bg,trk_wei[itrk]);
	  hVnS_FCal_bg[centb][iside][ihar]->Fill(NPT,NETA+1,corr2_FCal_bg*sign_eta,trk_wei[itrk]);
	  }
	  }
	  int str_size_Zdc = (Psi_Zdc_store[centb_1p][iside][ihar]).size();
	  if(str_size_Zdc>0){
	  for (int isiz=0; isiz<str_size_Zdc; isiz++) {
	  double Psi_Zdc_temp=(Psi_Zdc_store[centb_1p][iside][0]).at(isiz);
	  double corr1_Zdc_bg = cos( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc_temp) )*trk_wei[itrk];
	  double corr2_Zdc_bg = sin( (ihar+1)*(trk_phi0_wrt_PV[itrk]-Psi_Zdc_temp) )*trk_wei[itrk];
	  hVn_Zdc_bg[centb][iside][ihar]->Fill(ptBin,etaBin,corr1_Zdc_bg,trk_wei[itrk]);
	  hVn_Zdc_bg[centb][iside][ihar]->Fill(NPT,etaBin,corr1_Zdc_bg,trk_wei[itrk]);
	  hVn_Zdc_bg[centb][iside][ihar]->Fill(ptBin,NETA,corr1_Zdc_bg,trk_wei[itrk]);
	  hVn_Zdc_bg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr1_Zdc_bg*sign_eta,trk_wei[itrk]);
	  hVn_Zdc_bg[centb][iside][ihar]->Fill(NPT,NETA,corr1_Zdc_bg,trk_wei[itrk]);
	  hVn_Zdc_bg[centb][iside][ihar]->Fill(NPT,NETA+1,corr1_Zdc_bg*sign_eta,trk_wei[itrk]);

	  hVnS_Zdc_bg[centb][iside][ihar]->Fill(ptBin,etaBin,corr2_Zdc_bg,trk_wei[itrk]);
	  hVnS_Zdc_bg[centb][iside][ihar]->Fill(NPT,etaBin,corr2_Zdc_bg,trk_wei[itrk]);
	  hVnS_Zdc_bg[centb][iside][ihar]->Fill(ptBin,NETA,corr2_Zdc_bg,trk_wei[itrk]);
	  hVnS_Zdc_bg[centb][iside][ihar]->Fill(ptBin,NETA+1,corr2_Zdc_bg*sign_eta,trk_wei[itrk]);
	  hVnS_Zdc_bg[centb][iside][ihar]->Fill(NPT,NETA,corr2_Zdc_bg,trk_wei[itrk]);
	  hVnS_Zdc_bg[centb][iside][ihar]->Fill(NPT,NETA+1,corr2_Zdc_bg*sign_eta,trk_wei[itrk]);
	  }
	  }
	*/
      }
