...
void extractor_flat::Run_res() {
  ...
  */
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	//EEMC
	if(iside==0) Psi_ECal[iside][ihar] = atan2(Qy[6][ihar],Qx[6][ihar])/double(ihar+2);
        if(iside==1) Psi_ECal[iside][ihar] = atan2(Qy[7][ihar],Qx[7][ihar])/double(ihar+2);
        if(iside==2) Psi_ECal[iside][ihar] = atan2(Qy[6][ihar]+Qy[7][ihar],Qx[6][ihar]+Qx[7][ihar])/double(ihar+2);
	...
	if(dPsi_Zdc > (2.*ihar2+1)*PI && dPsi_Zdc <= (2.*ihar2+3)*PI) dPsi_Zdc-=2.*(ihar2+1)*PI; 
      ...
      */
      ...
      double dPsi_EF[4],dPsi_EZ[4],dPsi_FZ[4];
      int cnt=0;
      for(int i=0;i<2;i++){
	for(int j=0;j<2;j++){
	  dPsi_EF[cnt]=2.*(Psi_ECal[i][0]-Psi_FCal[j][0]);
	  dPsi_EZ[cnt]=2.*(Psi_ECal[i][0]-Psi_Zdc[j][0]);
	  dPsi_FZ[cnt]=2.*(Psi_FCal[i][0]-Psi_Zdc[j][0]);
	  hCorrCent_EF_fg[ihar][cnt]->Fill(centb,cos(dPsi_EF[cnt]));
	  hCorrCent_EZ_fg[ihar][cnt]->Fill(centb,cos(dPsi_EZ[cnt]));
	  hCorrCent_FZ_fg[ihar][cnt]->Fill(centb,cos(dPsi_FZ[cnt]));
	  cnt++;
	}
      }
      double dPsi_EE=2.*(Psi_ECal[0][0]-Psi_ECal[1][0]);
      double dPsi_FF=2.*(Psi_FCal[0][0]-Psi_FCal[1][0]);
      double dPsi_ZZ=2.*(Psi_Zdc[0][0]-Psi_Zdc[1][0]);
      hCorrCent_EE_fg[ihar]->Fill(centb,cos(dPsi_EE));
      hCorrCent_FF_fg[ihar]->Fill(centb,cos(dPsi_FF));
      hCorrCent_ZZ_fg[ihar]->Fill(centb,cos(dPsi_ZZ));
      /*
      //3-SubEvent resolution
      //Z1 is ZDC_A, F1 is FCAL_A, Z2 isZDC_C and F2 is FCAL_C 
      double dPsi_F1Z1=2*(Psi_FCal[0][0]-Psi_Zdc[0][0]);
      double dPsi_F1Z2=2*(Psi_FCal[0][0]-Psi_Zdc[1][0]);
      double dPsi_F1E1=2*(Psi_FCal[0][0]-Psi_ECal[0][0]);
      double dPsi_F1E2=2*(Psi_FCal[0][0]-Psi_ECal[1][0]);
      double dPsi_F1F2=2*(Psi_FCal[0][0]-Psi_FCal[1][0]);
      double dPsi_F2Z1=2*(Psi_FCal[1][0]-Psi_Zdc[0][0]);
      double dPsi_F2Z2=2*(Psi_FCal[1][0]-Psi_Zdc[1][0]);
      double dPsi_F2E1=2*(Psi_FCal[1][0]-Psi_ECal[0][0]);
      double dPsi_F2E2=2*(Psi_FCal[1][0]-Psi_ECal[1][0]);
      double dPsi_F2F1=2*(Psi_FCal[1][0]-Psi_FCal[0][0]);
      double dPsi_Z1F1=2*(Psi_Zdc[0][0]-Psi_FCal[0][0]);
      double dPsi_Z1F2=2*(Psi_Zdc[0][0]-Psi_FCal[1][0]);
      double dPsi_Z1E1=2*(Psi_Zdc[0][0]-Psi_ECal[0][0]);
      double dPsi_Z1E2=2*(Psi_Zdc[0][0]-Psi_ECal[1][0]);
      double dPsi_Z1Z2=2*(Psi_Zdc[0][0]-Psi_Zdc[1][0]);
      double dPsi_Z2F1=2*(Psi_Zdc[1][0]-Psi_FCal[0][0]);
      double dPsi_Z2F2=2*(Psi_Zdc[1][0]-Psi_FCal[1][0]);
      double dPsi_Z2E1=2*(Psi_Zdc[1][0]-Psi_ECal[0][0]);
      double dPsi_Z2E2=2*(Psi_Zdc[1][0]-Psi_ECal[1][0]);
      double dPsi_Z2Z1=2*(Psi_Zdc[1][0]-Psi_Zdc[0][0]);
      double dPsi_E1F1=2*(Psi_ECal[0][0]-Psi_FCal[0][0]);
      double dPsi_E1F2=2*(Psi_ECal[0][0]-Psi_FCal[1][0]);
      double dPsi_E1Z1=2*(Psi_ECal[0][0]-Psi_Zdc[0][0]);
      double dPsi_E1Z2=2*(Psi_ECal[0][0]-Psi_Zdc[1][0]);
      double dPsi_E1E2=2*(Psi_ECal[0][0]-Psi_ECal[1][0]);
      double dPsi_E2F1=2*(Psi_ECal[1][0]-Psi_FCal[0][0]);
      double dPsi_E2F2=2*(Psi_ECal[1][0]-Psi_FCal[1][0]);
      double dPsi_E2Z1=2*(Psi_ECal[1][0]-Psi_Zdc[0][0]);
      double dPsi_E2Z2=2*(Psi_ECal[1][0]-Psi_Zdc[1][0]);
      double dPsi_E2E1=2*(Psi_ECal[1][0]-Psi_ECal[0][0]);

      hResCent_F1Z1_fg[ihar]->Fill(centb,cos(dPsi_F1Z1));
      hResCent_F1Z2_fg[ihar]->Fill(centb,cos(dPsi_F1Z2));
      hResCent_F1E1_fg[ihar]->Fill(centb,cos(dPsi_F1E1));
      hResCent_F1E2_fg[ihar]->Fill(centb,cos(dPsi_F1E2));
      hResCent_F1F2_fg[ihar]->Fill(centb,cos(dPsi_F1F2));
      hResCent_F2Z1_fg[ihar]->Fill(centb,cos(dPsi_F2Z1));
      hResCent_F2Z2_fg[ihar]->Fill(centb,cos(dPsi_F2Z2));      
      hResCent_F2E1_fg[ihar]->Fill(centb,cos(dPsi_F2E1));
      hResCent_F2E2_fg[ihar]->Fill(centb,cos(dPsi_F2E2));
      hResCent_F2F1_fg[ihar]->Fill(centb,cos(dPsi_F2F1));
      hResCent_Z1F1_fg[ihar]->Fill(centb,cos(dPsi_Z1F1));
      hResCent_Z1F2_fg[ihar]->Fill(centb,cos(dPsi_Z1F2));
      hResCent_Z1E1_fg[ihar]->Fill(centb,cos(dPsi_Z1E1));
      hResCent_Z1E2_fg[ihar]->Fill(centb,cos(dPsi_Z1E2));
      hResCent_Z1Z2_fg[ihar]->Fill(centb,cos(dPsi_Z1Z2));
      hResCent_Z2F1_fg[ihar]->Fill(centb,cos(dPsi_Z2F1));
      hResCent_Z2F2_fg[ihar]->Fill(centb,cos(dPsi_Z2F2));
      hResCent_Z2E1_fg[ihar]->Fill(centb,cos(dPsi_Z2E1));
      hResCent_Z2E2_fg[ihar]->Fill(centb,cos(dPsi_Z2E2));
      hResCent_Z2Z1_fg[ihar]->Fill(centb,cos(dPsi_Z2Z1));
      hResCent_E1F1_fg[ihar]->Fill(centb,cos(dPsi_E1F1));
      hResCent_E1F2_fg[ihar]->Fill(centb,cos(dPsi_E1F2));
      hResCent_E1Z1_fg[ihar]->Fill(centb,cos(dPsi_E1Z1));
      hResCent_E1Z2_fg[ihar]->Fill(centb,cos(dPsi_E1Z2));
      hResCent_E1E2_fg[ihar]->Fill(centb,cos(dPsi_E1E2));
      hResCent_E2F1_fg[ihar]->Fill(centb,cos(dPsi_E2F1));
      hResCent_E2F2_fg[ihar]->Fill(centb,cos(dPsi_E2F2));
      hResCent_E2Z1_fg[ihar]->Fill(centb,cos(dPsi_E2Z1));
      hResCent_E2Z2_fg[ihar]->Fill(centb,cos(dPsi_E2Z2));
      hResCent_E2E1_fg[ihar]->Fill(centb,cos(dPsi_E2E1));
      */
...
void extractor_flat::Init_res() {
  ...
  */
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
    for(int i=0;i<4;i++){
      sprintf(name, "hCorrCent_EF_fg_har%d_ij%d",ihar,i);
      hCorrCent_EF_fg[ihar][i]= new TProfile(name, "", NCENT, 0, NCENT);
      sprintf(name, "hCorrCent_EZ_fg_har%d_ij%d",ihar,i);
      hCorrCent_EZ_fg[ihar][i]= new TProfile(name, "", NCENT, 0, NCENT);
      sprintf(name, "hCorrCent_FZ_fg_har%d_ij%d",ihar,i);
      hCorrCent_FZ_fg[ihar][i]= new TProfile(name, "", NCENT, 0, NCENT);
    }
    sprintf(name, "hCorrCent_EE_fg_har%d",ihar);
    hCorrCent_EE_fg[ihar]= new TProfile(name, "", NCENT, 0, NCENT);
    ...
    /*
    sprintf(name, "hResCent_F1Z1_fg_har%d", ihar);
    hResCent_F1Z1_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F1Z2_fg_har%d", ihar);
    hResCent_F1Z2_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F1F2_fg_har%d", ihar);
    hResCent_F1F2_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F2Z1_fg_har%d", ihar);
    hResCent_F2Z1_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F2Z2_fg_har%d", ihar);
    hResCent_F2Z2_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F2F1_fg_har%d", ihar);
    hResCent_F2F1_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);

    sprintf(name, "hResCent_Z1F1_fg_har%d", ihar);
    hResCent_Z1F1_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z1F2_fg_har%d", ihar);
    hResCent_Z1F2_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z1Z2_fg_har%d", ihar);
    hResCent_Z1Z2_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z2F1_fg_har%d", ihar);
    hResCent_Z2F1_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z2F2_fg_har%d", ihar);
    hResCent_Z2F2_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z2Z1_fg_har%d", ihar);
    hResCent_Z2Z1_fg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);

    sprintf(name, "hResCent_F1Z1_bg_har%d", ihar);
    hResCent_F1Z1_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F1Z2_bg_har%d", ihar);
    hResCent_F1Z2_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F1F2_bg_har%d", ihar);
    hResCent_F1F2_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F2Z1_bg_har%d", ihar);
    hResCent_F2Z1_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F2Z2_bg_har%d", ihar);
    hResCent_F2Z2_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_F2F1_bg_har%d", ihar);
    hResCent_F2F1_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);

    sprintf(name, "hResCent_Z1F1_bg_har%d", ihar);
    hResCent_Z1F1_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z1F2_bg_har%d", ihar);
    hResCent_Z1F2_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z1Z2_bg_har%d", ihar);
    hResCent_Z1Z2_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z2F1_bg_har%d", ihar);
    hResCent_Z2F1_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z2F2_bg_har%d", ihar);
    hResCent_Z2F2_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    sprintf(name, "hResCent_Z2Z1_bg_har%d", ihar);
    hResCent_Z2Z1_bg[ihar] = new TProfile(name, "", NCENT, 0, NCENT);
    */
  }
...
void extractor_flat::SaveHistos_res() {
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
    for(int i=0;i<4;i++){
      hCorrCent_EF_fg[ihar][i]->Write();
      hCorrCent_EZ_fg[ihar][i]->Write();
      hCorrCent_FZ_fg[ihar][i]->Write();
    }
    hCorrCent_EE_fg[ihar]->Write();
    hCorrCent_FF_fg[ihar]->Write();
    hCorrCent_ZZ_fg[ihar]->Write();
      /*
    hResCent_F1Z1_fg[ihar]->Write();
    hResCent_F1Z2_fg[ihar]->Write();
    hResCent_F1F2_fg[ihar]->Write();
    hResCent_F2Z1_fg[ihar]->Write();
    hResCent_F2Z2_fg[ihar]->Write();
    hResCent_F2F1_fg[ihar]->Write();

    hResCent_Z1F1_fg[ihar]->Write();
    hResCent_Z1F2_fg[ihar]->Write();
    hResCent_Z1Z2_fg[ihar]->Write();
    hResCent_Z2F1_fg[ihar]->Write();
    hResCent_Z2F2_fg[ihar]->Write();
    hResCent_Z2Z1_fg[ihar]->Write();
      */
  }
