...
void plot(){
  ...
  //3SubEvent
  TH1D *hCorrCent_Z1F1_fg[NHAR],*hCorrCent_Z1F2_fg[NHAR],*hCorrCent_Z1Z2_fg[NHAR];
  TH1D *hCorrCent_Z2F1_fg[NHAR],*hCorrCent_Z2F2_fg[NHAR],*hCorrCent_Z2Z1_fg[NHAR];
  TH1D *hCorrCent_F1Z1_fg[NHAR],*hCorrCent_F1Z2_fg[NHAR],*hCorrCent_F1F2_fg[NHAR];
  TH1D *hCorrCent_F2Z1_fg[NHAR],*hCorrCent_F2Z2_fg[NHAR],*hCorrCent_F2F1_fg[NHAR];

  TH1D *hCorrCent_Z1F1_bg[NHAR],*hCorrCent_Z1F2_bg[NHAR],*hCorrCent_Z1Z2_bg[NHAR];
  TH1D *hCorrCent_Z2F1_bg[NHAR],*hCorrCent_Z2F2_bg[NHAR],*hCorrCent_Z2Z1_bg[NHAR];
  TH1D *hCorrCent_F1Z1_bg[NHAR],*hCorrCent_F1Z2_bg[NHAR],*hCorrCent_F1F2_bg[NHAR];
  TH1D *hCorrCent_F2Z1_bg[NHAR],*hCorrCent_F2Z2_bg[NHAR],*hCorrCent_F2F1_bg[NHAR];

  TH1D *hCorrCent_EF_fg[NHAR][4],*hCorrCent_EZ_fg[NHAR][4],*hCorrCent_FZ_fg[NHAR][4];
  TH1D *hCorrCent_EE_fg[NHAR],*hCorrCent_FF_fg[NHAR],*hCorrCent_ZZ_fg[NHAR];
  TH1D *hCorrCent_EF_bg[NHAR][4],*hCorrCent_EZ_bg[NHAR][4],*hCorrCent_FZ_bg[NHAR][4];
  TH1D *hCorrCent_EE_bg[NHAR],*hCorrCent_FF_bg[NHAR],*hCorrCent_ZZ_bg[NHAR];
  ...
  //2. Get hDPsi
  fin2=new TFile("../zdcvn11/Output_res5.root","read");
  for (int icent=0; icent<NCENT; icent++) {
    for (int ihar=0; ihar<NHAR; ihar++) {
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
    for(int i=0;i<4;i++){
      sprintf(name,"hCorrCent2_EF_fg_har%d_ij%d",ihar,i);
      hCorrCent_EF_fg[ihar][i]=new TH1D(name,"",NCENT,cent_mat_double);
      sprintf(name,"hCorrCent2_EZ_fg_har%d_ij%d",ihar,i);
      hCorrCent_EZ_fg[ihar][i]=new TH1D(name,"",NCENT,cent_mat_double);
      sprintf(name,"hCorrCent2_FZ_fg_har%d_ij%d",ihar,i);
      hCorrCent_FZ_fg[ihar][i]=new TH1D(name,"",NCENT,cent_mat_double);
    }
    sprintf(name,"hCorrCent2_EE_fg_har%d",ihar);
    hCorrCent_EE_fg[ihar]=new TH1D(name,"",NCENT,cent_mat_double);
    ...
    for(int i=0;i<4;i++){
      ...
      htmp= (TProfile*)fin2->Get(name);
      GetTH1D(htmp,hCorrCent_EF_fg[ihar][i]);  
      sprintf(name, "hCorrCent_EZ_fg_har%d_ij%d",ihar,i);
      htmp= (TProfile*)fin2->Get(name);
      GetTH1D(htmp,hCorrCent_EZ_fg[ihar][i]);  
      sprintf(name, "hCorrCent_FZ_fg_har%d_ij%d",ihar,i);
      htmp= (TProfile*)fin2->Get(name);
      GetTH1D(htmp,hCorrCent_FZ_fg[ihar][i]);  
    }
    
    sprintf(name, "hCorrCent_EE_fg_har%d",ihar);
    htmp= (TProfile*)fin2->Get(name);
    GetTH1D(htmp,hCorrCent_EE_fg[ihar]);
    ...
    for (int icent=0; icent<NCENT; icent++) {
      ...
      y1=hCorrCent_EF_fg[ihar][0]->GetBinContent(icent+1);//E1F1
      dy1=hCorrCent_EF_fg[ihar][0]->GetBinError(icent+1);
      y2=hCorrCent_FF_fg[ihar]->GetBinContent(icent+1);//F1F2
      dy2=hCorrCent_FF_fg[ihar]->GetBinError(icent+1);
      y3=hCorrCent_EF_fg[ihar][1]->GetBinContent(icent+1);//E1F2
      dy3=hCorrCent_EF_fg[ihar][1]->GetBinError(icent+1);
      y=y1*y2/y3;
      ...
      y1=hCorrCent_FZ_fg[ihar][0]->GetBinContent(icent+1);//F1Z1
      dy1=hCorrCent_FZ_fg[ihar][0]->GetBinError(icent+1);
      y2=hCorrCent_FF_fg[ihar]->GetBinContent(icent+1);//F1F2
      dy2=hCorrCent_FF_fg[ihar]->GetBinError(icent+1);
      y3=hCorrCent_FZ_fg[ihar][2]->GetBinContent(icent+1);//F2Z1
      dy3=hCorrCent_FZ_fg[ihar][2]->GetBinError(icent+1);
      y=y1*y2/y3;
      dy=fabs(y1*y2/y3)*sqrt(pow(dy1/y1,2)+pow(dy2/y2,2)+pow(dy3/y3,2));
      //y=sqrt(fabs(y1*y2/y3));
      //dy=0.5*pow(fabs(y1*y2/y3),-0.5)*fabs(y1*y2/y3)*sqrt(pow(dy1/y1,2)+pow(dy2/y2,2)+pow(dy3/y3,2));
      ...
      y3=hCorrCent_FZ_fg[ihar][1]->GetBinContent(icent+1);//F1Z2
      dy3=hCorrCent_FZ_fg[ihar][1]->GetBinError(icent+1);
      y=y1*y2/y3;
      dy=fabs(y1*y2/y3)*sqrt(pow(dy1/y1,2)+pow(dy2/y2,2)+pow(dy3/y3,2));
      //y=sqrt(fabs(y1*y2/y3));
      //dy=0.5*pow(fabs(y1*y2/y3),-0.5)*fabs(y1*y2/y3)*sqrt(pow(dy1/y1,2)+pow(dy2/y2,2)+pow(dy3/y3,2));
      ...
      y1=hCorrCent_FZ_fg[ihar][1]->GetBinContent(icent+1);//F1Z2
      dy1=hCorrCent_FZ_fg[ihar][1]->GetBinError(icent+1);
      y2=hCorrCent_ZZ_fg[ihar]->GetBinContent(icent+1);//Z1Z2
      dy2=hCorrCent_ZZ_fg[ihar]->GetBinError(icent+1);
      y3=hCorrCent_FZ_fg[ihar][0]->GetBinContent(icent+1);//F1Z1
      dy3=hCorrCent_FZ_fg[ihar][0]->GetBinError(icent+1);
      y=y1*y2/y3;
      dy=fabs(y1*y2/y3)*sqrt(pow(dy1/y1,2)+pow(dy2/y2,2)+pow(dy3/y3,2));
      //y=sqrt(fabs(y1*y2/y3));
      //dy=0.5*pow(fabs(y1*y2/y3),-0.5)*fabs(y1*y2/y3)*sqrt(pow(dy1/y1,2)+pow(dy2/y2,2)+pow(dy3/y3,2));
  ...
  hCorrCent_bg_FCal[0]->SetYTitle("<cos2(#Psi_{2}^{A}-#Psi_{2}^{C})>");
  ...
  hCorrCent_fg_Zdc[0]->GetYaxis()->SetRangeUser(-0.1,0.);
  ...
  hCorrCent_bg_Zdc[0]->SetYTitle("<cos(#Psi_{1}^{A}-#Psi_{1}^{C})>");
  ...
  hResCent_bg_FCal[0]->SetYTitle("Resolution");
  ...
  hResCent_bg_Zdc[0]->SetYTitle("Resolution");
  ...
  //Plot hResCent for 3SubEvent
  TCanvas *c3;
  sprintf(name,"c3");
  c3=new TCanvas(name,"",1200,600);
  c3->Divide(2);
  TLegend *leg3[2];
  for(int i=0;i<2;i++){
    leg3[i]=new TLegend(0.65,0.7,0.88,0.88);
    leg3[i]->SetLineColor(0);
  }
  
  setStyle1(hCorrCent_3SE_FCal_fg[0][0],2);
  setStyle1(hCorrCent_3SE_FCal_fg[0][1],1);
  setStyle1(hCorrCent_FF_fg[0],0);
  setStyle1(hCorrCent_3SE_Zdc_fg[0][0],2);
  setStyle1(hCorrCent_3SE_Zdc_fg[0][1],1);
  setStyle1(hCorrCent_ZZ_fg[0],0);

  setStyle1(hCorrCent_FCal[0],0);
  setStyle1(hCorrCent_Zdc[1],0);
  setStyle1(hResCent_FCal[0],0);
  setStyle1(hResCent_Zdc[1],0);
  
  c3->cd(1);
  hCorrCent_3SE_FCal_fg[0][0]->Draw();
  hCorrCent_3SE_FCal_fg[0][1]->Draw("same");
  hCorrCent_FF_fg[0]->Draw("same");
  //hCorrCent_FCal[0]->Draw("same");
  hCorrCent_3SE_FCal_fg[0][0]->GetYaxis()->SetRangeUser(0.,1.);
  hCorrCent_3SE_FCal_fg[0][0]->SetXTitle("Centrality");
  hCorrCent_3SE_FCal_fg[0][0]->SetYTitle("<cos2(#Psi_{2}^{A}-#Psi_{2}^{C})>");
  hCorrCent_3SE_FCal_fg[0][0]->GetXaxis()->SetTitleOffset(1.5);
  hCorrCent_3SE_FCal_fg[0][0]->GetYaxis()->SetTitleOffset(1.6);
  leg3[0]->AddEntry(hCorrCent_3SE_FCal_fg[0][0],"3SE-F1E1F2","P");
  leg3[0]->AddEntry(hCorrCent_3SE_FCal_fg[0][1],"3SE-F1Z1F2","P");
  leg3[0]->AddEntry(hCorrCent_FF_fg[0],"2SE-F1F2","P");
  //leg3[0]->AddEntry(hCorrCent_FCal[0],"2SE-F1F2","P");
  leg3[0]->Draw("same");
  
  c3->cd(2);
  hCorrCent_3SE_Zdc_fg[0][0]->Draw();
  hCorrCent_3SE_Zdc_fg[0][1]->Draw("same");
  hCorrCent_ZZ_fg[0]->Draw("same");
  //hCorrCent_Zdc[1]->Draw("same");
  hCorrCent_3SE_Zdc_fg[0][0]->GetYaxis()->SetRangeUser(0.,0.01);
  hCorrCent_3SE_Zdc_fg[0][0]->SetXTitle("Centrality");
  hCorrCent_3SE_Zdc_fg[0][0]->SetYTitle("<cos2(#Psi_{2}^{A}-#Psi_{2}^{C})>");
  hCorrCent_3SE_Zdc_fg[0][0]->GetXaxis()->SetTitleOffset(1.5);
  hCorrCent_3SE_Zdc_fg[0][0]->GetYaxis()->SetTitleOffset(1.6);
  leg3[1]->AddEntry(hCorrCent_3SE_Zdc_fg[0][0],"3SE-Z1F1Z2","P");
  leg3[1]->AddEntry(hCorrCent_3SE_Zdc_fg[0][1],"3SE-Z2F1Z1","P");
  leg3[1]->AddEntry(hCorrCent_ZZ_fg[0],"2SE-Z1Z2","P");
  //leg3[1]->AddEntry(hCorrCent_Zdc[1],"2SE-Z1Z2","P");
  leg3[1]->Draw("same");
  
  sprintf(name,"./plots/Res_3SE_v%d.pdf",ihar+2);
  c3->SaveAs(name);
