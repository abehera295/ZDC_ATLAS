...
TH1D *hVnNeg_cent_raw[NHAR][NPT+2][NETA+4],*hVnNeg_cent_res[NHAR][NPT+2][NETA+4];
...
TH1D *hVnNeg_eta_raw[NCENT][NHAR][NPT+2],*hVnNeg_eta_res[NCENT][NHAR][NPT+2];
...
TH1D *hVnNeg_pt_raw_allCent[NHAR][NETA+4],*hVnNeg_pt_res_allCent[NHAR][NETA+4];
...
void FindEvenOdd2(TH1D *hEven,TH1D *hOdd,TH1D *h1,TH1D *h2){
  hEven->Reset();
  hOdd->Reset();
  h2->Reset();
  int N=h1->GetNbinsX();
  ...
  for(int i=0;i<N;i++){
    y1=h1->GetBinContent(i+1);
    dy1=h1->GetBinError(i+1);
    h2->SetBinContent(N-i,y1);
    h2->SetBinError(N-i,dy1);
  }
...
void plot_alice3(){
  TFile *fin1=new TFile("./zdcvn8/Output_SP.root","read");
  ...
  TProfile *hMean_pt_allCent[NSIDE+1];
  for (int iside=0; iside<NSIDE+1; iside++) {
  ...
  //v1(eta)
  TH1D *hposA,*hposC,*hnegA,*hnegC;
  TH1D *hE,*hO,*hEA,*hEC,*hOA,*hOC;

  hposA=(TH1D*)hVn_Zdc_eta_res[2][0][ihar][NPT]->Clone();
  hnegA=(TH1D*)hVn_Zdc_eta_res[2][0][ihar][NPT]->Clone();
  hEA=(TH1D*)hVn_Zdc_eta_res[2][0][ihar][NPT]->Clone();
  hOA=(TH1D*)hVn_Zdc_eta_res[2][0][ihar][NPT]->Clone();
  FindEvenOdd2(hEA,hOA,hposA,hnegA);

  hposC=(TH1D*)hVn_Zdc_eta_res[2][1][ihar][NPT]->Clone();
  hnegC=(TH1D*)hVn_Zdc_eta_res[2][1][ihar][NPT]->Clone();
  hEC=(TH1D*)hVn_Zdc_eta_res[2][1][ihar][NPT]->Clone();
  hOC=(TH1D*)hVn_Zdc_eta_res[2][1][ihar][NPT]->Clone();
  FindEvenOdd2(hEC,hOC,hposC,hnegC);
  ...
  */
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
	...
	sprintf(name, "hV%dEven_cent_raw_pt%d_eta%d", ihar+1, ipt,ieta);
        hVnEven_cent_raw[ihar][ipt][ieta]=(TH1D*)hVn_Zdc_cent_raw[0][ihar][ipt][ieta]->Clone(name);
	sprintf(name, "hV%dOdd_cent_raw_pt%d_eta%d", ihar+1, ipt,ieta);
        hVnOdd_cent_raw[ihar][ipt][ieta]=(TH1D*)hVn_Zdc_cent_raw[0][ihar][ipt][ieta]->Clone(name);
	...
	sprintf(name, "hV%dEven_cent_res_pt%d_eta%d", ihar+1, ipt,ieta);
        hVnEven_cent_res[ihar][ipt][ieta]=(TH1D*)hVn_Zdc_cent_res[0][ihar][ipt][ieta]->Clone(name);
	sprintf(name, "hV%dOdd_cent_res_pt%d_eta%d", ihar+1, ipt,ieta);
        hVnOdd_cent_res[ihar][ipt][ieta]=(TH1D*)hVn_Zdc_cent_res[0][ihar][ipt][ieta]->Clone(name);
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
    for(int icent=0;icent<NCENT;icent++){
      for (int ipt=0; ipt<NPT+1; ipt++) {
	sprintf(name, "hV%dEven_cent_raw_cent%d_pt%d", ihar+1,icent,ipt);
        hVnEven_eta_raw[icent][ihar][ipt]=(TH1D*)hVn_Zdc_eta_raw[icent][NSIDE][ihar][ipt]->Clone(name);
	sprintf(name, "hV%dOdd_cent_raw_cent%d_pt%d", ihar+1,icent,ipt);
        hVnOdd_eta_raw[icent][ihar][ipt]=(TH1D*)hVn_Zdc_eta_raw[icent][NSIDE][ihar][ipt]->Clone(name);
	sprintf(name, "hV%dNeg_cent_raw_cent%d_pt%d", ihar+1,icent,ipt);
        hVnNeg_eta_raw[icent][ihar][ipt]=(TH1D*)hVn_Zdc_eta_raw[icent][NSIDE][ihar][ipt]->Clone(name);

	FindEvenOdd2(hVnEven_eta_raw[icent][ihar][ipt],hVnOdd_eta_raw[icent][ihar][ipt],hVn_Zdc_eta_raw[icent][NSIDE][ihar][ipt],hVnNeg_eta_raw[icent][ihar][ipt]);

	sprintf(name, "hV%dEven_cent_res_cent%d_pt%d", ihar+1,icent,ipt);
        hVnEven_eta_res[icent][ihar][ipt]=(TH1D*)hVn_Zdc_eta_res[icent][NSIDE][ihar][ipt]->Clone(name);
	sprintf(name, "hV%dOdd_cent_res_cent%d_pt%d", ihar+1,icent,ipt);
        hVnOdd_eta_res[icent][ihar][ipt]=(TH1D*)hVn_Zdc_eta_res[icent][NSIDE][ihar][ipt]->Clone(name);
	sprintf(name, "hV%dNeg_cent_res_cent%d_pt%d", ihar+1,icent,ipt);
        hVnNeg_eta_res[icent][ihar][ipt]=(TH1D*)hVn_Zdc_eta_res[icent][NSIDE][ihar][ipt]->Clone(name);

	FindEvenOdd2(hVnEven_eta_res[icent][ihar][ipt],hVnOdd_eta_res[icent][ihar][ipt],hVn_Zdc_eta_res[icent][NSIDE][ihar][ipt],hVnNeg_eta_res[icent][ihar][ipt]);
      }
    }
  }
  ...
  TLegend *leg1;
  leg1=new TLegend(0.15,0.7,0.4,0.85);
  //leg1->SetNColumns(2);
  leg1->SetLineColor(0);
  ...
  TLegend *leg2;
  leg2=new TLegend(0.15,0.7,0.4,0.85);
  //leg2->SetNColumns(2);
  leg2->SetLineColor(0);
  ...
  gVnOdd_pt_res_allCent[ihar][ieta2]->GetYaxis()->SetRangeUser(-0.002,0.005);
  ...
  TLegend *leg3;
  leg3=new TLegend(0.15,0.7,0.48,0.88);
  //leg3->SetNColumns(2);
  leg3->SetLineColor(0);
  leg3->AddEntry( hVnEven_cent_res[ihar][ipt][ieta1],"v_{1} Even ATLAS (p_{T}>0.4GeV)","P");
  leg3->AddEntry(hVnOdd_cent_res[ihar][ipt][ieta2],"v_{1} Odd ATLAS (p_{T}>0.4GeV)","P");
  ...
  hposA=(TH1D*)hVn_Zdc_eta_res[icent1][0][ihar][ipt]->Clone();
  hnegA=(TH1D*)hVn_Zdc_eta_res[icent1][0][ihar][ipt]->Clone();
  hEA=(TH1D*)hVn_Zdc_eta_res[icent1][0][ihar][ipt]->Clone();
  hOA=(TH1D*)hVn_Zdc_eta_res[icent1][0][ihar][ipt]->Clone();
  FindEvenOdd2(hEA,hOA,hposA,hnegA);

  hposC=(TH1D*)hVn_Zdc_eta_res[icent1][1][ihar][ipt]->Clone();
  hnegC=(TH1D*)hVn_Zdc_eta_res[icent1][1][ihar][ipt]->Clone();
  hEC=(TH1D*)hVn_Zdc_eta_res[icent1][1][ihar][ipt]->Clone();
  hOC=(TH1D*)hVn_Zdc_eta_res[icent1][1][ihar][ipt]->Clone();
  FindEvenOdd2(hEC,hOC,hposC,hnegC);
  ...
  */
  ...
  TLegend *leg4;
  leg4=new TLegend(0.5,0.7,0.88,0.88);
  //leg4->SetNColumns(2);
  leg4->SetLineColor(0);
  leg4->AddEntry( hVnEven_eta_res[icent1][ihar][ipt],"v_{1} Even ATLAS (p_{T}>0.4GeV)","P");
  leg4->AddEntry(hVnOdd_eta_res[icent1][ihar][ipt],"v_{1} Odd ATLAS (p_{T}>0.4GeV)","P");
  ...
  c3->cd(1);
  hVnOdd_eta_res[icent1][ihar][ipt]->Draw();
  //hVnOdd_eta_res[icent1][ihar][ipt]->GetXaxis()->SetRangeUser(-0.8,0.8);
  hVnOdd_eta_res[icent1][ihar][ipt]->GetXaxis()->SetTitle("#eta");
  hVnOdd_eta_res[icent1][ihar][ipt]->GetYaxis()->SetTitle("v_{1}");
  hVnEven_eta_res[icent1][ihar][ipt]->Draw("same");
  ...
  TLegend *leg5;
  leg5=new TLegend(0.5,0.7,0.88,0.88);
  //leg5->SetNColumns(2);
  leg5->SetLineColor(0);
  leg5->AddEntry( hVnEven_eta_res[icent2][ihar][ipt],"v_{1} Even ATLAS (p_{T}>0.4GeV)","P");
  leg5->AddEntry(hVnOdd_eta_res[icent2][ihar][ipt],"v_{1} Odd ATLAS (p_{T}>0.4GeV)","P");
  ...
  line0_3->Draw("same");
  /*
  hVnOdd_eta_res[icent2][ihar][ipt]->Draw();
  //hVnOdd_eta_res[icent2][ihar][ipt]->GetXaxis()->SetRangeUser(-0.8,0.8);
  hVnOdd_eta_res[icent2][ihar][ipt]->GetXaxis()->SetTitle("#eta");
  hVnOdd_eta_res[icent2][ihar][ipt]->GetYaxis()->SetTitle("v_{1}");
  hVnEven_eta_res[icent2][ihar][ipt]->Draw("same");
