...
extractor_flat::extractor_flat(string filelist,int first, int last, int iev, int nevts) {
  from=first;
  to=last;
  NEv_id=iev;
  NEv=nevts;
  sprintf(name,"./out/out_%d_%d_%d.root",from,to,NEv_id);
  outname=name;
