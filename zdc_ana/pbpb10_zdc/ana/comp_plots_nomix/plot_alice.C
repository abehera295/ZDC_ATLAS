...
void plot_alice(){
...
#ifdef withBG
  TFile *fin2=new TFile("../zdcvn11/Output_doAna_vn_mix2.root","read");
#endif
#ifndef withBG
  TFile *fin2=new TFile("../zdcvn11/Output_doAna_vn_nomix.root","read");
#endif
  for (int icent=0; icent<NCENT; icent++) {
    for (int iside=0; iside<NSIDE+1; iside++) {
      sprintf(name, "hMean_pt_cent%d_ch%d_side%d",icent,2,iside);
      hMean_pt[icent][2][iside]=(TProfile*)fin2->Get(name);
      sprintf(name, "hMean_eta_cent%d_ch%d_side%d",icent,2,iside);
      hMean_eta[icent][2][iside]=(TProfile*)fin2->Get(name);
    }
  }
  ...
  for (int ihar=0; ihar<NHAR; ihar++) {
	...
	hVn_Zdc_cent_raw[iside][ihar][NPT][ieta]=(TH1D*)fin2->Get(name);
	sprintf(name, "hV%d_Zdc_cent_res_side%d_pt%d_eta%d", ihar+1,iside,NPT,ieta);
	hVn_Zdc_cent_res[iside][ihar][NPT][ieta]=(TH1D*)fin2->Get(name);
      }
      for(int icent=0;icent<NCENT;icent++){
	sprintf(name, "hV%d_Zdc_eta_raw_cent%d_side%d_pt%d", ihar+1,icent,iside,NPT);
	hVn_Zdc_eta_raw[icent][iside][ihar][NPT]=(TH1D*)fin2->Get(name);
	sprintf(name, "hV%d_Zdc_eta_res_cent%d_side%d_pt%d", ihar+1,icent,iside,NPT);
	hVn_Zdc_eta_res[icent][iside][ihar][NPT]=(TH1D*)fin2->Get(name);
      }
    }
  }
