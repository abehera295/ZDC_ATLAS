...
void extractor_flat::ReadHistos_vn() {
  ...
  for(int id=0;id<NDET;id++){
    for(int iz=0;iz<NZ;iz++){
      ...
      hQw_m[id][iz]=(TProfile*)fin1->Get(name);
...
void extractor_flat::Run_vn() {
  ...
  for(int id=0;id<NDET;id++){
    for(int iz=0;iz<NZ;iz++){
      for(int ic=0;ic<NCENT_1p;ic++){
	Qwmean[id][iz][ic] = hQw_m[id][iz]->GetBinContent(ic+1);
      }
      for(int ihar=0;ihar<NHAR;ihar++){//2,3,4,5
	//hqx[id][iz][ihar]->Divide(hQx[id][iz][ihar],hQw_m[id][iz]);
	//hqy[id][iz][ihar]->Divide(hQy[id][iz][ihar],hQw_m[id][iz]);
	for(int ic=0;ic<NCENT_1p;ic++){
